...
bool isWhiteFile(const char* c_szFileName)
{
	for (int i = 0; i < ARRAYSIZE(white_file_list); i++) {
		if (stricmp(c_szFileName, white_file_list[i]) == 0) { return true; }
	}
	return false;
}
...
bool CEterPackManager::Get(CMappedFile & rMappedFile, const char * c_szFileName, LPCVOID * pData)
{
	...
	if (m_iSearchMode == SEARCH_FILE) {
		if (GetFromFile(rMappedFile, c_szFileName, pData)) { return true; }
		return GetFromPack(rMappedFile, c_szFileName, pData);
	}
	else {
		if (isExistInPack(c_szFileName)) { return GetFromPack(rMappedFile, c_szFileName, pData); }
		else if (isExist(c_szFileName)) { return GetFromFile(rMappedFile, c_szFileName, pData); }
	}
	return false;
#else
...
bool CEterPackManager::GetFromPack(CMappedFile & rMappedFile, const char * c_szFileName, LPCVOID * pData)
{
	...
	if (m_pFoxFS) {
		int errorCodeSize = 0;
		if ((errorCodeSize = Crypto_ExistsA(m_pFoxFS, c_szFileName)) == FoxFS::ERROR_OK) {
			unsigned int dwSize = Crypto_SizeA(m_pFoxFS, c_szFileName), dwReadSize = 0;
			BYTE* pbData = new BYTE[dwSize + 1];
			int errorCode = 0;
			if ((errorCode = Crypto_GetA(m_pFoxFS, c_szFileName, pbData, dwSize, &dwReadSize)) == FoxFS::ERROR_OK) {
				pbData[dwReadSize] = 0;
				*pData = pbData;
	...
	else { TraceError("FoxFS: Not initialized!"); }
	return false;
#else
	FinderLock lock(m_csFinder);
...
bool CEterPackManager::isExistInPack(const char * c_szFileName)
{
	...
	if (m_pFoxFS) {
		int errorCodeSize = 0;
		if ((errorCodeSize = Crypto_ExistsA(m_pFoxFS, c_szFileName)) == FoxFS::ERROR_OK) { return true; }
		else { Tracenf("FoxFS :  File not exists %s with error code %d", c_szFileName, errorCodeSize); }
	}
	else { TraceError("FoxFS: Not initialized!"); }
...
bool CEterPackManager::isExist(const char * c_szFileName)
{
	...
	if (m_iSearchMode == SEARCH_PACK) {
		if (isWhiteFile(c_szFileName) || isWhiteFileExt(c_szFileName)) { return isExistInPack(c_szFileName) || (_access(c_szFileName, 0) == 0); }
		return isExistInPack(c_szFileName);
	}
	if (_access(c_szFileName, 0) == 0) return true;
	return isExistInPack(c_szFileName);
#endif
...
void CEterPackManager::RegisterRootPack(const char * c_szName)
{
	...
	if (m_pFoxFS) {
		int errorCode = 0;
		if ((errorCode = Crypto_LoadA(m_pFoxFS, c_szName)) != FoxFS::ERROR_OK) { TraceError("%s: Error Code %d", c_szName, errorCode); }
	}
	else { TraceError("FoxFS: Not initialized!"); }
	return;
#endif
