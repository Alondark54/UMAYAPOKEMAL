...
void CPythonPlayer::NEW_RefreshMouseWalkingDirection()
{
	...
	urielac::CheckReservedMode(m_eReservedMode);
#endif
	switch (m_eReservedMode)
	{
		case MODE_CLICK_ITEM:
		{
			...
			if (rkIT.GetGroundItemPosition(m_dwIIDReserved, &kPPosPickedItem))
			{
				...
				{
#ifdef URIEL_ANTI_CHEAT
	urielac::SetMoveEvent((DWORD*)&kPPosPickedItem.x, (DWORD)_ReturnAddress());
#endif
		...
		case MODE_CLICK_POSITION:
		{
			...
			{
#ifdef URIEL_ANTI_CHEAT
	urielac::SetMoveEvent((DWORD*)&m_kPPosReserved.x, (DWORD)_ReturnAddress());
#endif
...
#ifdef URIEL_ANTI_CHEAT
	urielac::CheckAttackKeyState(m_isAtkKey);
#endif
	if (m_isAtkKey)
		NEW_Attack();
