...
class CPythonPlayer : public CSingleton<CPythonPlayer>, public IAbstractPlayer
{
	...
	protected:
		std::map<BYTE, std::vector<TRanking>> ranking_data;
	public:
		void		SetRankData(BYTE rankIndex, long long value);
		void		SetRankData(BYTE rankIndex, std::vector<TRanking>& vector);
		TRanking* GetRankData(BYTE rankIndex, BYTE rankList);
#endif
...
protected:
	...
	bool m_bAutoHuntMob, m_bAutoHuntStone;
#endif
