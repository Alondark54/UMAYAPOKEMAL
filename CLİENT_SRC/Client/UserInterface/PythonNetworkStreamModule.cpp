...
PyObject* netSendLuckyDrawCurrentPacket(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendLuckyDrawCurrent();
	return Py_BuildNone();
}
PyObject* netSendLuckyDrawAddParcitipant(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendLuckyDrawAddParcitipant();
	return Py_BuildNone();
}
PyObject* netSendLuckyDrawClaimAward(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendLuckyDrawClaimAward();
	return Py_BuildNone();
}
#endif
...
PyObject* netSendItemShopPacket(PyObject* poSelf, PyObject* poArgs) {
	BYTE subHeader, itemCount; DWORD itemID; char* szQuery; bool withQuery;
	if (!PyTuple_GetInteger(poArgs, 0, &subHeader)) { return Py_BuildException(); }
	if (!PyTuple_GetInteger(poArgs, 1, &itemCount)) { return Py_BuildException(); }
	if (!PyTuple_GetUnsignedLong(poArgs, 2, &itemID)) { return Py_BuildException(); }
	if (!PyTuple_GetString(poArgs, 3, &szQuery)) { return Py_BuildException(); }
	if (!PyTuple_GetBoolean(poArgs, 4, &withQuery)) { return Py_BuildException(); }
...
PyObject* netSendExtBattlePassComplateMission(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bType, bBattlePassID, bMissionIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &bType)) { return Py_BuildException(); }
	if (!PyTuple_GetInteger(poArgs, 1, &bBattlePassID)) { return Py_BuildException(); }
	if (!PyTuple_GetInteger(poArgs, 2, &bMissionIndex)) { return Py_BuildException(); }
	
	CPythonNetworkStream::Instance().SendExtBattlePassComplateMission(bType, bBattlePassID, bMissionIndex);
	return Py_BuildNone();
}
#endif
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "SendExtBattlePassComplateMission",	netSendExtBattlePassComplateMission,	METH_VARARGS },
#endif
#ifdef ENABLE_LUCKY_DRAW
		{ "SendLuckyDrawCurrent",						netSendLuckyDrawCurrentPacket,						METH_VARARGS },
		{ "SendLuckyDrawAddParcitipant",				netSendLuckyDrawAddParcitipant,						METH_VARARGS },
		{ "SendLuckyDrawClaimAward",				netSendLuckyDrawClaimAward,						METH_VARARGS },
#endif
		{ NULL,										NULL,										NULL },
	};
	...
	PyModule_AddIntConstant(poModule, "IS_ITEM_SOCKET_MAX", IS_ITEM_SOCKET_MAX);
	PyModule_AddIntConstant(poModule, "IS_ITEM_ATTR_MAX", IS_ITEM_ATTR_MAX);
#endif
}
