...
bool CPythonNetworkStream::SendItemMovePacket(TItemPos pos, TItemPos change_pos, BYTE num)
{
	...
	TraceError("pos : %d - change pos : %d", pos.cell, change_pos.cell);
...
bool CPythonNetworkStream::SendRefreshOfflineShop(int iDelay)
{
	if (iDelay > 0) {
		...
		if (timeGetTime() - s_LastTime < iDelay) return true;
...
bool CPythonNetworkStream::SendOfflineShopChangeItemPrice(BYTE bDisplayPos, long long llPrice)
{
	...
	TPacketGCShopUpdatePrice pTable;
	pTable.dwShopVid = 0;
	pTable.bPos = bDisplayPos;
	pTable.iElkAmount = llPrice;

	if (!Send(sizeof(TPacketGCShopUpdatePrice), &pTable)) {
		Tracef("SendOfflineShopChangeItemPrice Error\n");
		return false;
	}
