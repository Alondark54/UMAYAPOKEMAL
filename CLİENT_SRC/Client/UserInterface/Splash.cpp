...
unsigned int __stdcall CSplashWnd::SplashThreadProc(void* lpParameter)
{
	...
	pThis->m_hSplashWnd = CreateWindowExW(pThis->m_WindowName.length() ? 0 : WS_EX_TOOLWINDOW, L"SplashWnd", pThis->m_WindowName.c_str(),
	...
	while ((bRet = GetMessage(&msg, NULL, 0, 0)) != 0)
	{
		...
		{
			SendMessage(pThis->m_hSplashWnd, PBM_SETPOS, LOWORD(msg.wParam), 0);
		...
		if (msg.message == PBM_SETBARCOLOR)
		{
			if (!IsWindow(pThis->m_hProgressWnd)) {
				SendMessage(pThis->m_hSplashWnd, PBM_SETPOS, 0, 0); 
			}
...
{
	wchar_t* msg;
	int len = ::LoadStringW(hModule, nResourceID, reinterpret_cast<wchar_t*>(&msg), 0);
