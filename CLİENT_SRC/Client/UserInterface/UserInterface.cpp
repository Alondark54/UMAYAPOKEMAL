...
static const std::vector <std::string> packFiles = {
	"updates",
	"icon",
	"item",
	"locale",
...
bool isFileFound(const std::string& szName) {
	std::string fileName = "pack/"; fileName += szName; fileName += ".lica";
	struct stat buffer;
	auto isFound = (stat(fileName.c_str(), &buffer) == 0);
	if (!isFound) {
...
bool PackInitialize(const char * c_pszFolder)
{
#ifdef ENABLE_FS
	NANOBEGIN
		if (_access(c_pszFolder, 0) != 0) return true;
	...
	char packName[64];
	for (const auto& packFile : packFiles) { if (isFileFound(packFile)) {
		snprintf(packName, sizeof(packName), "pack/%s.lica", packFile.c_str());
		CEterPackManager::Instance().RegisterPack(packName, "*"); }
	}
	CEterPackManager::Instance().RegisterRootPack((stFolder + std::string("root.lica")).c_str());
	NANOEND
		return true;
#else
	NANOBEGIN
...
#ifdef ENABLE_BYKATIL199_LOGO_LOGIN
		CSplash splash1(TEXT("mazi2.bmp"), RGB(128, 128, 128));
		splash1.ShowSplash();
		Sleep(2000);
		splash1.CloseSplash();
#endif
