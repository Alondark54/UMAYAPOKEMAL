...
void CPythonPlayer::SetRankData(BYTE rankIndex, long long value)
{
	auto it = ranking_data.find(rankIndex);
	if (it != ranking_data.end())
	{
		for (DWORD j = 0; j < it->second.size(); ++j)
		{
			if (it->second[j].index == 11)
				it->second[j].value = value;
		}
	}
}
void CPythonPlayer::SetRankData(BYTE rankIndex, std::vector<TRanking>& vector)
{
	auto it = ranking_data.find(rankIndex);
	if (it != ranking_data.end())
	{
		...
		if (vector.size())
		{
			for (BYTE j = 0; j < vector.size(); ++j)
			{
				if (vector[j].empire > 0)
					it->second.emplace_back(vector[j]);
			}
		}
	}
	else
	{
		if (vector.size())
		{
			std::vector<TRanking> new_data;
			new_data.clear();
			for (BYTE j = 0; j < vector.size(); ++j)
			{
				if (vector[j].empire > 0)
					new_data.emplace_back(vector[j]);
			}
			ranking_data.emplace(rankIndex, new_data);
		}
	}
}

TRanking* CPythonPlayer::GetRankData(BYTE rankIndex, BYTE rankList)
{
	auto it = ranking_data.find(rankIndex);
	if (it != ranking_data.end())
	{
...
void CPythonPlayer::Clear()
{
	...
	urielac::SetReservedMode(MODE_NONE, (DWORD)_ReturnAddress());
#endif
