...
PyObject* playerGetRankInfo(PyObject* poSelf, PyObject* poArgs)
{
	BYTE szCategory;
	if (!PyTuple_GetByte(poArgs, 0, &szCategory))
		return Py_BadArgument();
	BYTE szRankList;
	if (!PyTuple_GetByte(poArgs, 1, &szRankList))
		return Py_BadArgument();
	TRanking* data = CPythonPlayer::Instance().GetRankData(szCategory, szRankList);
	if (data == NULL)
		return Py_BuildValue("siL", "", 0, 0);
	return Py_BuildValue("siL", data->name, data->empire, data->value);
}
#endif
...
void initPlayer()
{
...
#ifdef __RANKING_SYSTEM__
		{ "GetRankInfo", playerGetRankInfo, METH_VARARGS },
#endif
...
#ifdef __RANKING_SYSTEM__
	PyModule_AddIntConstant(poModule, "RANK_LEVEL", RANK_LEVEL);
	PyModule_AddIntConstant(poModule, "RANK_STONE", RANK_STONE);
	PyModule_AddIntConstant(poModule, "RANK_MONSTER", RANK_MONSTER);
	PyModule_AddIntConstant(poModule, "RANK_BOSS", RANK_BOSS);
	PyModule_AddIntConstant(poModule, "RANK_DUNGEON", RANK_DUNGEON);
	PyModule_AddIntConstant(poModule, "RANK_PLAYTIME", RANK_PLAYTIME);
	PyModule_AddIntConstant(poModule, "RANK_GOLD", RANK_GOLD);
	PyModule_AddIntConstant(poModule, "RANK_GAYA", RANK_GAYA);
	PyModule_AddIntConstant(poModule, "RANK_CHEST", RANK_CHEST);
#endif
	...
	PyModule_AddIntConstant(poModule, "EXINVENTORY_POS_START", c_ExInventory_PosStart);
	PyModule_AddIntConstant(poModule, "EXINVENTORY_POS_END", c_ExInventory_PosEnd);
