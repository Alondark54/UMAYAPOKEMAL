...
{
	int iVID;
	if (!PyTuple_GetInteger(poArgs, 0, &iVID)) return Py_BadArgument();
	...
	if (!pCharacterInstance) return Py_BuildNone();
	...
	rkpyMiniMap.DeleteTarget(9513571);
	rkpyMiniMap.CreateTarget(9513571, "OfflineShop", iVID);
	rkpyMiniMap.UpdateTarget(9513571, kPixelPosition.x, kPixelPosition.y);
	return Py_BuildNone();
}
...
PyObject* backgroundDeletePrivateShopPos(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonMiniMap::Instance().DeleteTarget(8899);
	return Py_BuildNone();
}
