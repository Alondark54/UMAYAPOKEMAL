...
void CInstanceBase::SetAttackSpeed(UINT uAtkSpd)
{
	...
	if (__IsMainInstance() && !urielac::SetAttackSpeed(uAtkSpd, GetVirtualID()))
		return;
#endif
...
#ifdef URIEL_ANTI_CHEAT
	urielac::SetMoveEvent((DWORD*)&kPPosDst.x, (DWORD)_ReturnAddress());
#endif
...
{
#ifdef URIEL_ANTI_CHEAT
	urielac::CheckMoveEvent((DWORD*)&c_rkPPosDst.x);
#endif
	if (__IsSyncing())
	{
		return false;
