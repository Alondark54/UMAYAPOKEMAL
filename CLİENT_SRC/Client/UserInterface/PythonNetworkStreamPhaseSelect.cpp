...
void CPythonNetworkStream::SelectPhase()
{
	...
	case HEADER_GC_PLAYER_PIN_CODE:
		if (__RecvPlayerPinCodePacket())
			return;
		break;
#endif
		///////////////////////////////////////////////////////////////////////////////////////////
...
#if defined(ENABLE_PLAYER_PIN_SYSTEM)
bool CPythonNetworkStream::SendCreateCharacterPacket(BYTE index, const char* name, BYTE job, BYTE shape, BYTE byCON, BYTE byINT, BYTE bySTR, BYTE byDEX, const char* pin)
#else
...
#if defined(ENABLE_PLAYER_PIN_SYSTEM)
	strncpy(createCharacterPacket.pin, pin, PIN_CODE_LENGTH);
#endif
...
#if defined(ENABLE_PLAYER_PIN_SYSTEM)
bool CPythonNetworkStream::SendCharacterPinCodePacket(BYTE bIndex, const char* c_szPinCode)
{
	TPacketCGCharacterPinCode CharacterPinCodePacket;

	CharacterPinCodePacket.bHeader = HEADER_CG_PLAYER_PIN_CODE;
	CharacterPinCodePacket.bIndex = bIndex;
	strncpy(CharacterPinCodePacket.szPinCode, c_szPinCode, PIN_CODE_LENGTH + 1);

	if (!Send(sizeof(TPacketCGCharacterPinCode), &CharacterPinCodePacket))
	{
		Tracen("SendCharacterPinCodePacket - Error");
		return false;
	}
...
bool CPythonNetworkStream::__RecvPlayerPinCodePacket()
{
	TPacketGCCharacterPinCode CharacterPinCodePacket;
	if (!Recv(sizeof(TPacketGCCharacterPinCode), &CharacterPinCodePacket))
		return false;

	m_bVerifiedPIN = CharacterPinCodePacket.bVerified;

	if (CharacterPinCodePacket.bVerified)
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_SELECT], "OnPinCodeSuccess", Py_BuildValue("()"));
		return true;
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_SELECT], "OnPinCodeFailure", Py_BuildValue("()"));
	return true;
}
#endif
