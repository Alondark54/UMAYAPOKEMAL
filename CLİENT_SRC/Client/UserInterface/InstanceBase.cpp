...
BOOL CInstanceBase::IsInWater(CInstanceBase& rkInstDst)
{
	for (int bX = -1; bX <= 1; ++bX) {
		for (int yX = -1; yX <= 1; ++yX) {
			const TPixelPosition& c_rkPPosCur = rkInstDst.NEW_GetCurPixelPositionRef();
			if (CPythonBackground::Instance().isAttrOn(c_rkPPosCur.x + bX*100, c_rkPPosCur.y + yX*100, (1 << 1)))
				return TRUE;
		}
	}
	return FALSE;
...
bool CInstanceBase::IsAttackableInstance(CInstanceBase& rkInstVictim)
{
	...
	else if (IsPC())
	{
		...
		if (rkInstVictim.IsPC())
		{
			if (rkInstVictim.IsFishingMode() && (IsInWater(rkInstVictim)))
				return false;
			if (GetDuelMode())
			{
				switch(GetDuelMode())
...
{
	if (GetRace() == 30000 || (GetRace() >= 30002 && GetRace() <= 30008)) return true;
	return false;
}
