...
void CPythonNetworkStream::GamePhase()
{
    ...
    while (ret)
	{
		...
		switch (header)
		{
			...
			case HEADER_GC_OFFLINE_SHOP: ret = RecvOfflineShopPacket(); break;
			case HEADER_GC_OFFLINE_SHOP_SIGN: ret = RecvOfflineShopSignPacket(); break;
			case HEADER_GC_OFFLINE_SHOP_OPEN_PANEL: ret = RecvOfflineShopOpenPanelPacket(); break;
			case HEADER_GC_OFFLINE_SHOP_START_P2P: ret = RecvOfflineShopStartP2PPacket(); break;
			case HEADER_GC_OFFLINE_CHANGE_SHOP_SIGN: ret = RecvOfflineShopChangeSignPacket(); break;
			case HEADER_GC_OFFLINE_CHANGE_SHOP_SIGN_P2P: ret = RecvOfflineShopChangeSignPacketP2P(); break;
			case HEADER_GC_OFFLINE_ADVISE_PLAYER_BUY: ret = RecvOfflineShopPlayerBuyItem(); break;
#endif
			...
			case HEADER_GC_LUCKY_DRAW_CURRENT_LD:
				ret = RecvLuckyDrawCurrent();
				break;
#endif
...
#ifdef RUNIK_ULTIMATE
	if (!ult_0x0459y7x(0x757a8b46, 0x93907c90, &kStatePacket.runikhash))
		return false;
#endif
...
bool CPythonNetworkStream::RecvBiologPacket()
{
	...
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "Binary_BioUpdate", Py_BuildValue("(iiiiiiiiiiiiiii)",
		p.dwItemVnum, p.dwSoulVnum, p.bGivenCount, p.bState, p.bRequiredCount, p.bAffect[0], p.bAffValue[0],
		p.bAffect[1], p.bAffValue[1],
		p.bAffect[2], p.bAffValue[2],
		p.bAffect[3], p.bAffValue[3],
		p.bChance, p.iTime));
...
#ifdef ENABLE_LUCKY_DRAW
bool CPythonNetworkStream::RecvLuckyDrawCurrent()
{
	TPacketGCLuckyDrawCurrentLD tLuckyDrawCurrent;
	if (!Recv(sizeof(TPacketGCLuckyDrawCurrentLD), &tLuckyDrawCurrent))
		return false;

	int* aAwardList = new int[48];
	int arraySize = 48; // Set this to the actual size of aAwardList

	for (int i = 0; i < 24; i++)
	{
		aAwardList[(i * 2)] = tLuckyDrawCurrent.luckyDrawTable.aAwardList[i].dVnum;
		aAwardList[(i * 2) + 1] = tLuckyDrawCurrent.luckyDrawTable.aAwardList[i].dCount;
	}

	// Create a Python list from the array
	PyObject* pyList = PyList_New(arraySize);
	for (int i = 0; i < arraySize; ++i) {
		PyList_SetItem(pyList, i, PyLong_FromLong(aAwardList[i]));
	}
	...
	PyCallClassMemberFunc(
		...
		"OpenLuckyDrawDialog",
		Py_BuildValue("(iiiiiiiiOsssii)",
			tLuckyDrawCurrent.luckyDrawTable.dJoinItemVnum,
			tLuckyDrawCurrent.luckyDrawTable.dJoinItemCount,
			tLuckyDrawCurrent.luckyDrawTable.lEnterPrice,
			tLuckyDrawCurrent.luckyDrawTable.dEndTime,
			tLuckyDrawCurrent.luckyDrawTable.dTotalParticipant,
			tLuckyDrawCurrent.luckyDrawTable.dMaxParticipant,
			tLuckyDrawCurrent.luckyDrawTable.dParticipantPerPlayer,
			tLuckyDrawCurrent.playerParticipantCount,
			pyList,
			&tLuckyDrawCurrent.luckyDrawTable.winner1Name,
			&tLuckyDrawCurrent.luckyDrawTable.winner2Name,
			&tLuckyDrawCurrent.luckyDrawTable.winner3Name,
			tLuckyDrawCurrent.luckyDrawTable.bIsFinished,
			tLuckyDrawCurrent.isWon
		)
	);

	// Decrease reference count for pyList if it is no longer needed
	Py_DECREF(pyList);
	return true;
}

bool CPythonNetworkStream::SendLuckyDrawCurrent()
{
	TPacketCGLuckyDrawCurrentLD luckyDrawPacket;
	luckyDrawPacket.header = HEADER_CG_LUCKY_DRAW_CURRENT_LD;

	if (!Send(sizeof(luckyDrawPacket), &luckyDrawPacket))
		return false;

	return SendSequence();
}

bool CPythonNetworkStream::SendLuckyDrawAddParcitipant()
{
	TPacketCGLuckyDrawAddParticipant luckyDrawPacket;
	luckyDrawPacket.header = HEADER_CG_LUCKY_DRAW_ADD_PARTICIPANT;
...
bool CPythonNetworkStream::SendLuckyDrawClaimAward()
{
	TPacketCGLuckyDrawAddParticipant luckyDrawPacket;
	luckyDrawPacket.header = HEADER_CG_LUCKY_DRAW_CLAIM_AWARD;
...
bool CPythonNetworkStream::RecvPrivateShopSearch()
{
	...
	for (; iPacketSize > 0; iPacketSize -= sizeof(TPacketGCPrivateShopSearchItem))
	{
		...
		//TraceError("RecvPrivateShopSearch() itemVnum: %d | seller: %s | vid: %d|%d", SShopSearch->item.vnum, SShopSearch->name.c_str(), Item.dwShopPID, SShopSearch->dwShopPID);
...
bool CPythonNetworkStream::SendBuildOfflineShopPacket(const char* c_szName, const std::vector<TOfflineShopItemTable>& c_rSellingItemStock, BYTE bNpcType, BYTE bBoardStyle)
{
	...
	if (!Send(sizeof(packet), &packet)) return false;
	for (std::vector<TOfflineShopItemTable>::const_iterator itor = c_rSellingItemStock.begin(); itor < c_rSellingItemStock.end(); ++itor) {
		...
		if (!Send(sizeof(c_rItem), &c_rItem)) return false;
	}
...
bool CPythonNetworkStream::RecvOfflineShopSignPacket()
{
	...
	if (pInstance) {
		if (pInstance->IsShop()) {
			if (0 == strlen(p.szSign)) PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_OfflineShop_Disappear", Py_BuildValue("(i)", p.dwVID));
			else PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_OfflineShop_Appear", Py_BuildValue("(isi)", p.dwVID, p.szSign, p.bBorderStyle));
		}
		else
...
bool CPythonNetworkStream::RecvOfflineShopOpenPanelPacket()
{
	...
	if (!Recv(sizeof(TPacketGCOpenOffShopPanel), &p)) return false;
...
bool CPythonNetworkStream::RecvOfflineShopStartP2PPacket()
{
	...
	if (!Recv(sizeof(TPacketGCOfflineShopStartP2P), &pShopStartPacket)) return false;
...
bool CPythonNetworkStream::RecvOfflineShopChangeSignPacket()
{
	...
	if (pInstance) {
		if (pInstance->IsShop()) {
			if (0 == strlen(p.szSign)) PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_OfflineShop_Disappear", Py_BuildValue("(i)", p.dwVID));
			else PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_OfflineShop_ChangeSign", Py_BuildValue("(is)", p.dwVID, p.szSign));
		}
		else
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_OfflineShop_Disappear", Py_BuildValue("(i)", p.dwVID));
	}
...
bool CPythonNetworkStream::RecvOfflineShopPlayerBuyItem()
{
	...
	if (!Recv(sizeof(TPacketGCOfflineShopAdviseOwnerSell), &p)) return false;
...
bool CPythonNetworkStream::RecvOfflineShopPacket()
{
	...
	TPacketGCShop  packet_shop;
	if (!Recv(sizeof(packet_shop), &packet_shop)) return false;

	int iSize = packet_shop.size - sizeof(packet_shop);
	if (iSize > 0) {
		vecBuffer.resize(iSize);
		if (!Recv(iSize, &vecBuffer[0])) return false;
	}
	...
	case SHOP_SUBHEADER_GC_START: {
		...
		for (BYTE iItemIndex = 0; iItemIndex < OFFLINE_SHOP_HOST_ITEM_MAX_NUM; ++iItemIndex) CPythonShop::Instance().SetOfflineShopItemData(iItemIndex, pShopStartPacket->items[iItemIndex]);
	...
	case SHOP_SUBHEADER_GC_UPDATE_PRICE: {
		...
		PyTuple_SetItem(args, 2, PyLong_FromLongLong(pShopUpdateItemPricePacket->iElkAmount));
	...
	case SHOP_SUBHEADER_GC_BUY_FROM_OWNSHOP:
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnShopError", Py_BuildValue("(s)", "BUY_FROM_OWNSHOP"));
		break;
	case SHOP_SUBHEADER_GC_REMOVED_FROM_SHOP:
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnShopError", Py_BuildValue("(s)", "REMOVED_FROM_SHOP"));
		break;
	case SHOP_SUBHEADER_GC_PRICE_CHANGED:
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnShopError", Py_BuildValue("(s)", "PRICE_CHANGED"));
		break;
	case SHOP_SUBHEADER_GC_YOU_ALREADY_HAVE_ONE:
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnShopError", Py_BuildValue("(s)", "YOU_ALREADY_HAVE_ONE"));
		break;
	...
	case SHOP_SUBHEADER_GC_CHECK_RESULT: {
		...
		if (!Recv(sizeof(BYTE), &bHasOfflineShop)) return false;
		CPythonPlayer::instance().SetHasOfflineShop(bHasOfflineShop == 1 ? true : false);
		if (bHasOfflineShop != 0) PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RefreshOfflineShopManager", Py_BuildValue("()"));
		else PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "CloseOfflineShopManager", Py_BuildValue("()"));
		break;
	}
...
bool CPythonNetworkStream::RecvItemShopCategoryPacket() {
	...
	if (!Recv(sizeof(gPacket), &gPacket)) { return false; }
	gPacket.dataSize -= sizeof(gPacket);
	WORD vIndex = 0;
	while (gPacket.dataSize > 0) {
		TGCIShopCategory catData;
		if (!Recv(sizeof(catData), &catData)) { return false; }
		gPacket.dataSize -= sizeof(catData);
		vIndex++;
		if (catData.mainID == 255) { PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "AddCategory", Py_BuildValue("(si)", catData.categoryName, catData.catID)); }
		else { PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "FillCategory", Py_BuildValue("(isi)", catData.mainID, catData.categoryName, catData.catID)); }
	}
...
bool CPythonNetworkStream::RecvItemShopItemsPacket() {
	...
	while (gPacket.dataSize > 0) {
		...
		if (!Recv(sizeof(itemData), &itemData)) { return false; }
		...
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "AddItem", Py_BuildValue("(iiiiissi)", itemData.iID, itemData.iVnum, itemData.iCount, itemData.iPrice, itemData.categoryID, szMetinStr, szAttrStr, itemData.isVoteCoins ? 1 : 0));
	}
...
#ifdef ENABLE_EXTENDED_BATTLE_PASS
bool CPythonNetworkStream::SendExtBattlePassComplateMission(BYTE bType, BYTE bBattlePassID, BYTE bMissionIndex)
{
	if (!__CanActMainInstance())
		return true;

	TPacketCGBattlePassComplateMission packet;
	packet.bHeader = HEADER_CG_EXT_SEND_BP_COMPLATE_MISSION;
	packet.bType = bType;
	packet.bBattlePassID = bBattlePassID;
	packet.bMissionIndex = bMissionIndex;

	if (!Send(sizeof(TPacketCGBattlePassComplateMission), &packet))
	{
		Tracef("SendExtBattlePassComplateMission Send Packet Error\n");
		return false;
	}
...
bool CPythonNetworkStream::RecvRankUpdate()
{
	...
	BYTE rankIndex;
	if (!Recv(sizeof(BYTE), &rankIndex))
		return false;

	if (subIndex == RANK_LOAD)
	{
		BYTE listSize;
		if (!Recv(sizeof(BYTE), &listSize))
			return false;
		std::vector<TRanking> tmpVec;
		tmpVec.clear();
		tmpVec.resize(listSize);
		if (!Recv(sizeof(TRanking) * listSize, &tmpVec[0]))
			return false;
		CPythonPlayer::Instance().SetRankData(rankIndex, tmpVec);
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "LoadRankData", Py_BuildValue("(ii)", rankIndex, 1));
	}
	else
	{
		long long rankValue;
		if (!Recv(sizeof(long long), &rankValue))
			return false;
		CPythonPlayer::Instance().SetRankData(rankIndex, rankValue);
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "LoadRankUpdate", Py_BuildValue("(i)", rankIndex));
	}
	return true;
}
#endif
