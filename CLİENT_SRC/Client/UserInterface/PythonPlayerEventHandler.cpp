...
//#define ATTACK_TIME_LOG
	...
	DWORD dwVIDVictimVerify = dwVIDVictim;
	
	if (rkActorVictim.GetActorType() != CActorInstance::TYPE_ENEMY && rkActorVictim.GetActorType() != CActorInstance::TYPE_STONE)
		dwVIDVictimVerify |= 0x80000000;

	/*const char* map_name = CPythonBackground::Instance().GetWarpMapName();
	if(strcmp(map_name, "<cikmasini istemediginiz map ismi>") == 0)
		dwVIDVictimVerify |= 0x80000000;*/
	
	if (!urielac::GetEncryptedTargetVID(&dwVIDVictimVerify, &rkActorVictim, 0xbd23b371, 0xc9e76d8))
		return;
	
	rkStream.SendAttackPacket(uSkill, dwVIDVictim, dwVIDVictimVerify);
#else
