...
void CClientManager::QUERY_ITEM_SAVE(CPeer * pkPeer, const char * c_pData)
{
	...
	if (p->window == SAFEBOX || p->window == MALL)
	{
		...
		snprintf(szQuery, sizeof(szQuery),
			...
			"attrtype5, attrvalue5, "
#ifdef ENABLE_SOULBIND_SYSTEM
			"attrtype6, attrvalue6, sealbind) "
			"VALUES(%u, %u, %d, %d, %u, %u, %ld, %ld, %ld, %ld, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %ld)",
#else
			"attrtype6, attrvalue6) "
...
void CClientManager::ProcessPackets(CPeer * peer)
{
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
			...
			case HEADER_GD_LUCKY_DRAW_ADD_PARTICIPANT:
				LuckyDrawManager::instance().AddParticipant(peer, dwHandle, (TPacketGDLuckyDrawAddParticipant*)data);
				break;
			case HEADER_GD_LUCKY_DRAW_CURRENT_LD:
				LuckyDrawManager::instance().GetCurrentLuckyDraw(peer);
				break;
#endif
...
int CClientManager::Process()
{
	...
	while (pulses--)
	{
		...
		if (!(thecore_heart->pulse % (thecore_heart->passes_per_sec * 10)))
		{
			LuckyDrawManager::instance().UpdateCurrentLuckyDraw();
		}
#endif
