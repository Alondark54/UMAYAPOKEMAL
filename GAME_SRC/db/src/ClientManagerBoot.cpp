...
void CClientManager::SendRank(CPeer* pkPeer)
{
	...
	BYTE rankTotalSize = rank_data.size();
	buf.write(&rankTotalSize, sizeof(BYTE));

	for (BYTE j = 0; j < RANK_MAX; j++)
	{
		auto it = rank_data.find(j);
		if (it != rank_data.end())
		{
			buf.write(&it->first, sizeof(BYTE));
			BYTE rankSize = it->second.size();
			buf.write(&rankSize, sizeof(BYTE));
			if(rankSize > 0)
				buf.write(it->second.data(), sizeof(TRanking)*rankSize);
		}
	}

	if (pkPeer != NULL)
	{
		pkPeer->EncodeHeader(HEADER_DG_RANK_UPDATE, 0, buf.size());
		pkPeer->Encode(buf.read_peek(), buf.size());
	}
	else
	{
		ForwardPacket(HEADER_DG_RANK_UPDATE, buf.read_peek(), buf.size());
	}
}
bool CClientManager::InitializeRank()
{
	rank_data.clear();
	std::map<BYTE, std::string> query_data = {
		{RANK_LEVEL,"SELECT account_id, name, level FROM player.player ORDER BY level DESC LIMIT 10" },
		{RANK_STONE,"SELECT account_id, name, killed_metin FROM player.player ORDER BY killed_metin DESC LIMIT 10" },
		{RANK_MONSTER,"SELECT account_id, name, killed_monster FROM player.player ORDER BY killed_monster DESC LIMIT 10" },
		{RANK_BOSS,"SELECT account_id, name, killed_boss FROM player.player ORDER BY killed_boss DESC LIMIT 10" },
		{RANK_DUNGEON,"SELECT account_id, name, success_dungeon FROM player.player ORDER BY success_dungeon DESC LIMIT 10" },
		{RANK_PLAYTIME,"SELECT account_id, name, playtime FROM player.player ORDER BY playtime DESC LIMIT 10" },
		{RANK_GOLD,"SELECT account_id, name, gold FROM player.player ORDER BY gold DESC LIMIT 10" },
		{RANK_GAYA,"SELECT account_id, name, gaya FROM player.player ORDER BY gaya DESC LIMIT 10" },
		{RANK_CHEST,"SELECT account_id, name, open_chest FROM player.player ORDER BY open_chest DESC LIMIT 10" },
	};
	for (BYTE j = 0; j < query_data.size(); ++j)
	{
		auto it = query_data.find(j);
		if (it == query_data.end())
			continue;
		std::vector<TRanking> rank_vec_data;
		rank_vec_data.clear();

		std::unique_ptr<SQLMsg> pMsg(CDBManager::Instance().DirectQuery(it->second.c_str()));
		int iLine = 0;
		MYSQL_ROW mRow;
		while (NULL != (mRow = mysql_fetch_row(pMsg->Get()->pSQLResult)))
		{
			...
			int iCur = 0;
			DWORD account_id = 0;
			str_to_number(account_id, mRow[iCur++]);
			char szQuery[256];
			snprintf(szQuery, sizeof(szQuery), "SELECT empire FROM player.player_index WHERE id = '%d';", account_id);
			std::unique_ptr<SQLMsg> pMsg2(CDBManager::instance().DirectQuery(szQuery));
			MYSQL_ROW  empire_row = mysql_fetch_row(pMsg2->Get()->pSQLResult);
			if (empire_row == NULL)
				continue;
			str_to_number(rank_item.empire, empire_row[0]);
			strlcpy(rank_item.name, mRow[iCur++], sizeof(rank_item.name));
			str_to_number(rank_item.value, mRow[iCur++]);
			rank_vec_data.emplace_back(rank_item);
			iLine++;
		}
		rank_data.emplace(j, rank_vec_data);
	}
	return true;
}
#endif
