...
size_t CreatePlayerSaveQuery(char * pszQuery, size_t querySize, TPlayerTable * pkTab)
{
...
#ifdef __RANKING_SYSTEM__
		,pkTab->killed_metin
		,pkTab->killed_monster
		,pkTab->killed_boss
		,pkTab->success_dungeon
		,pkTab->gaya
		,pkTab->caught_fishes
		,pkTab->open_chest
#endif
	...
	CDBManager::instance().EscapeString(text, pkTab->uiAutomaticItemProcess, sizeof(pkTab->uiAutomaticItemProcess));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, "item_process_info = '%s', ", text);
#endif
...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	else
	{
		...
		snprintf(queryStr, sizeof(queryStr),
				...
				"UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(last_play),horse_skill_point,bio_level,bio_given,bio_state,bio_time,bio_ntf,battle_pass_premium_id"
...
#endif
#ifdef __RANKING_SYSTEM__
	str_to_number(pkTab->killed_metin, row[col++]);
	str_to_number(pkTab->killed_monster, row[col++]);
	str_to_number(pkTab->killed_boss, row[col++]);
	str_to_number(pkTab->success_dungeon, row[col++]);
	str_to_number(pkTab->gaya, row[col++]);
	str_to_number(pkTab->caught_fishes, row[col++]);
	str_to_number(pkTab->open_chest, row[col++]);
#endif
#ifdef ENABLE_AUTOMATIC_ITEM_PROCESS
	if (row[col])
		thecore_memcpy(pkTab->uiAutomaticItemProcess, row[col], sizeof(pkTab->uiAutomaticItemProcess));
	else
		memset(&pkTab->uiAutomaticItemProcess, 0, sizeof(pkTab->uiAutomaticItemProcess));
