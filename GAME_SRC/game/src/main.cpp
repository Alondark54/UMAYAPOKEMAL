...
void heartbeat(LPHEART ht, int pulse)
{
	...
	if (!(pulse % ht->passes_per_sec))
	{
		...
		{
			LuckyDraw::instance().GetCurrentLuckyDrawFromDB();
		}
#endif
...
int main(int argc, char **argv)
{
...
#endif
#ifdef ENABLE_CAOS_EVENT
	CNewCaosEventManager	caosEventManager;
#endif

#ifdef ENABLE_AUTO_EVENTS
	CAutoEventSystem	EventManager;
#endif

#ifdef ENABLE_NEW_MISSIONS
	CNewMissions		newMissionMng;
#endif

#ifdef ENABLE_BOT_CONTROL
	CNewBotControl	botcontrolMng;
#endif
	if (!start(argc, argv)) {
		CleanUpForEarlyExit();
		return 0;
	...
	PanamaLoad();
#ifdef FATE_ROULETTE
	CFateRoulette	fateroulette;
	fateroulette.Initialize();
#endif
...
#ifdef ENABLE_NEW_MISSIONS
	if (!g_bAuthServer) { newMissionMng.Initialize(); }
#endif
