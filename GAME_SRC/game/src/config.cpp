...
MYSQL* mysql_connection_setup(struct connection_details mysql_details)
{
	...
	{
		printf("MangoFiles: Lisans Sunucusuna Baglanamiyor....");
		exit(1);
	}
	return connection;
}
...
void config_init(const string& st_localeServiceName)
{
	...
	mysqlD.server = "5.180.187.155";
	mysqlD.user = "metin2files_fileslisans";
	mysqlD.password = "Mkkkq9=TF[mY";
	mysqlD.database = "metin2files_fileslisans";

	conn = mysql_connection_setup(mysqlD);

	char verimiznedir[256];
	snprintf(verimiznedir, sizeof(verimiznedir), "SELECT COUNT(*) as count FROM Lisans WHERE ipaddress='%s'", g_szPublicIP);
	res = mysql_perform_query(conn, (verimiznedir));

	while ((row = mysql_fetch_row(res)) !=NULL)
		verilcount = row[0];
		if (verilcount != "1")
		{
			printf("MangoFiles: Lisans YOK Lutfen iletisime geciniz...IPAddress: %s\n", g_szPublicIP);
			exit(4);
		}
		else
		{
			printf("MangoFiles: Lisansli Kullanim icin tesekkur ederiz..IPAddress: %s\n", g_szPublicIP);
		}
#endif
