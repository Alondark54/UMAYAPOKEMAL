...
bool CMountActor::_UpdateFollowAI()
{
	...
	{
		float fOwnerRot = m_pkOwner->GetRotation() * 3.141592f / 280.f;
...
CMountSystem::CMountSystem(LPCHARACTER owner)
{
	...
	m_dwMountTimestamp = 0;
	m_dwUnmountTimestamp = 0;
}
...
{
	//if (m_pkOwner->IncreaseMountCounter() >= 2)
	//{
	//	m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "<Binek> Binege binmek icin 2 saniye beklemelisin.");
	//	return;
	//}
...
void CMountSystem::Mount(DWORD mobVnum, LPITEM mountItem)
{
    ...
    // Deðiþiklik: Binmeye izin verilmeden önce geçen süreyi kontrol edin
   	DWORD currentTime = get_dword_time();
	if (currentTime - m_dwMountTimestamp < PASSES_PER_SEC(2)) {
	// Ýzin verilen süre geçmedi, bu yüzden binme iþlemine izin verme
        	m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "<Binek> Binege binmek için 2 saniye beklemelisin.");
        	return;
    	}

    // Deðiþiklik: Binme iþlemi baþladýðý zamaný kaydet
    	m_dwMountTimestamp = currentTime;
...
void CMountSystem::Unmount(DWORD mobVnum)
{
	...
	DWORD currentTime = get_dword_time();
	if (currentTime - m_dwUnmountTimestamp < PASSES_PER_SEC(2)) {
		// Ýzin verilen süre geçmedi, bu yüzden geri inme iþlemine izin verme
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "<Binek> Binekten inmek için 2 saniye beklemelisin.");
		return;
	}

    // Deðiþiklik: Geri inme iþlemi baþladýðý zamaný kaydet
	m_dwUnmountTimestamp = currentTime;
