...
bool CItem::AddToCharacter(LPCHARACTER ch, TItemPos Cell)
{
	...
	{
		if (m_wCell >= EXINVENTORY_POS_END && BELT_INVENTORY_SLOT_START > m_wCell)
		{
...
int CItem::FindEquipCell(LPCHARACTER ch, int iCandidateCell)
{
	...
	else if (GetType() == ITEM_TYPE_NEW)
	{
		if (GetSubType() == TYPE_WEAR_NEW_SLOT1) 
		{
			return WEAR_NEW_SLOT1;
		
		}
		else if (GetSubType() == TYPE_WEAR_NEW_SLOT2) 
		{
			return WEAR_NEW_SLOT2;
		...
		case ITEM_ROD:
			{
				if (bAdd)
				{
					if (m_wCell == EXINVENTORY_POS_END + WEAR_WEAPON)
...
bool CItem::EquipTo(LPCHARACTER ch, BYTE bWearCell)
{
	...
	m_pOwner = ch;
	m_bEquipped = true;
	m_wCell	= EXINVENTORY_POS_END + bWearCell;
...
bool CItem::Unequip()
{
	if (!m_pOwner || GetCell() < EXINVENTORY_POS_END)
	{
	...
	if (this != m_pOwner->GetWear(GetCell() - EXINVENTORY_POS_END))
	{
   ...
   if (GetVnum() == DBONE_VNUM_2){m_pOwner->RemoveAffect(AFFECT_DBONE_2);}
	...
	m_pOwner->SetWear(GetCell() - EXINVENTORY_POS_END, NULL);
