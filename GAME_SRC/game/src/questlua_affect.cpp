...
namespace quest
{
	...
	ALUA(affect_add_battlepass)
	{
		...
		ch->AddAffect(AFFECT_BATTLEPASSS_AFFECT_IDX + (ch->GetQuestFlag("bp.affect") + 1), aApplyInfo[applyOn].bPointType, value, 0, duration, 0, false);
		return 0;
	}
	...
	void RegisterAffectFunctionTable()
	{
		...
		{
			{ "add",		affect_add		},
			{ "add_battlepass",		affect_add_battlepass		},
