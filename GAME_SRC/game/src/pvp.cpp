...
#ifdef ENABLE_CAOS_EVENT
	if (pkChr->GetMapIndex() == CAOS_EVENT_MAP && (quest::CQuestManager::instance().GetEventFlag("caos_event") == 0 || quest::CQuestManager::instance().GetEventFlag("caos_event") == 2 ))
	{
		pkChr->ChatPacket(1, "Su an bu haritada savas yapamazsiniz.");
		return false;
	}
#endif
