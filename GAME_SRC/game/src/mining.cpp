...
//	}
	...
	int GetFractionCount()
	{
    		return 1; // Sadece 1 adet eþya atýlacak
	}
	...
	void CHEAT_MAX_PICK(LPCHARACTER ch, LPITEM item)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Kazma deneyimi (%d) artti."), Pick_GetCurExp(pick));
	}
	...
	void PracticePick(LPCHARACTER ch, LPITEM item)
	{
		...
		if (Pick_IsPracticeSuccess(pick))
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Kazma en yuksek seviyede."));
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Kazman bir sonraki seviyeye yukseltilebilir."));
			}
			else
			{
				Pick_IncCurExp(pick);

				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Kazma exp kazandi! (%d/%d)"),
						Pick_GetCurExp(pick), Pick_GetMaxExp(pick));

				if (Pick_Refinable(pick))
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Kazma maksimum tecrubeye ulasti."));
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Gelistirmek icin Deokbae'ye git."));
				}
			}
		}
	}
	...
	EVENTFUNC(mining_event)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Kazman olmadan bunu yapamazsin."));
			return 0;
		}
		// END_OF_REFINE_PICK
		...
		if (!load)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu olmaz."));
			return 0;
		}
		...
		if (number(1, 100) <= iPct)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Madencilik basarili"));
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Madencilik basarisiz."));
		}
