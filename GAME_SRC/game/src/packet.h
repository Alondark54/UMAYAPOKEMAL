...
#endif
#ifdef ITEM_SHOP
	HEADER_CG_ITEMSHOP_PROCESS	= 219,
#endif

#ifdef ENABLE_EXTENDED_BATTLE_PASS
	HEADER_CG_EXT_BATTLE_PASS_ACTION			= 220,
	HEADER_CG_EXT_SEND_BP_PREMIUM_ITEM		= 221,
	HEADER_CG_EXT_SEND_BP_COMPLATE_MISSION		= 222,
#endif

#ifdef ENABLE_CUBE_RENEWAL_WORLDARD
	HEADER_CG_CUBE_RENEWAL 						= 223,
#endif

#ifdef ENABLE_LUCKY_DRAW
	HEADER_CG_LUCKY_DRAW_ADD_PARTICIPANT = 224,
	HEADER_CG_LUCKY_DRAW_CURRENT_LD = 225,
	HEADER_CG_LUCKY_DRAW_CLAIM_AWARD = 226,
#endif
...
#ifdef ITEM_SHOP
	HEADER_GC_ITEMSHOP_CATEGORY = 218,
	HEADER_GC_ITEMSHOP_ITEM = 219,
#endif

#ifdef ENABLE_EXTENDED_BATTLE_PASS
	HEADER_GC_EXT_BATTLE_PASS_OPEN 					= 220,
	HEADER_GC_EXT_BATTLE_PASS_GENERAL_INFO 		= 221,
	HEADER_GC_EXT_BATTLE_PASS_MISSION_INFO 		= 222,
	HEADER_GC_EXT_BATTLE_PASS_MISSION_UPDATE 	= 223,
	HEADER_GC_EXT_BATTLE_PASS_SEND_RANKING 		= 224,
#endif

#ifdef ENABLE_EVENT_MANAGER
	HEADER_GC_EVENT_MANAGER = 225,
#endif

#ifdef __RANKING_SYSTEM__
	HEADER_GC_RANKING_UPDATE				= 226, 
#endif

#ifdef ENABLE_LUCKY_DRAW
		HEADER_GC_LUCKY_DRAW_ADD_PARTICIPANT = 227,
		HEADER_GC_LUCKY_DRAW_CURRENT_LD = 228,
#endif
...
#endif
#ifdef __OFFLINE_PRIVATE_SHOP_SYSTEM__
	HEADER_GG_REMOVE_OFFLINE_SHOP,
	HEADER_GG_OFFLINE_SHOP_BUY,
	HEADER_GG_CHANGE_OFFLINE_SHOP_TIME,
	HEADER_GG_OFFLINE_SHOP_OPEN_PANEL,
	HEADER_GG_OFFLINE_SHOP_ADD_ITEM,
	HEADER_GG_OFFLINE_SHOP_REMOVE_ITEM,
	HEADER_GG_OFFLINE_SHOP_CLOSE_SHOP,
	HEADER_GG_OFFLINE_SHOP_CHANGE_NAME,
	HEADER_GG_OFFLINE_SHOP_ADVISE_PLAYER,
	HEADER_GG_OFFLINE_SHOP_CHANGE_ITEM_PRICE,
#endif
#ifdef NEW_SALES_SYSTEM
	HEADER_GG_SALES					= 42, //// buradaki 46 gg packet listesindeki en son rakam olmalı ve başka packet kullanmamalı
#endif
#ifdef FATE_ROULETTE
	HEADER_GG_FATE_ROULLETTE					= 43,
#endif
#ifdef ENABLE_CAOS_EVENT
	HEADER_GG_CAOS_EVENT = 45, // sayi varsa degistir
#endif
#ifdef ENABLE_NEW_MISSIONS
	HEADER_GG_NEWMISSIONS = 46,
	HEADER_GG_NEWMISSIONS_NEW_PLAYER = 47,
	HEADER_GG_GLOBAL_MISSIONS = 48,
#endif

#ifdef ENABLE_REWARD_SYSTEM
	HEADER_GG_REWARD_INFO = 51,
#endif
...
typedef struct SPacketGGLogin
{
	...
	char	bIpAdress[IP_ADDRESS_LENGTH + 1];
	bool	bMultiFarmStatus;
#endif
} TPacketGGLogin;
...
#ifdef ENABLE_LUCKY_DRAW
typedef struct SPacketCGLuckyDrawAddParticipant
{
	BYTE header;
} TPacketCGLuckyDrawAddParticipant;

typedef struct SPacketGCLuckyDrawAddParticipant
{
	BYTE header;
	BYTE subheader;
} TPacketGCLuckyDrawAddParticipant;

typedef struct SPacketCGLuckyDrawCurrentLD
{
	BYTE header;
} TPacketCGLuckyDrawCurrentLD;

typedef struct SPacketCGLuckyDrawClaimAward
{
	BYTE header;
} TPacketCGLuckyDrawClaimAward;

typedef struct SPacketGCLuckyDrawCurrentLD
{
	BYTE header;
	TLuckyDrawTable luckyDrawTable;
	DWORD playerParticipantCount;
	bool isWon;
} TPacketGCLuckyDrawCurrentLD;
...
#ifdef ENABLE_NEW_MISSIONS
typedef struct SPacketGGNewMissions
{
	BYTE	bHeader;
	DWORD	dwID;
	BYTE	bMissionType;
	...
	DWORD	target_vnum;
	BYTE	index;
} TPacketGGNewMissions;
typedef struct SPacketGGNewMissionsCharacter
{
	BYTE	bHeader;
	DWORD	dwID;
	BYTE	missionType;
	BYTE	index;
} TPacketGGNewMissionsCharacter;
typedef struct SPacketGGNewGlobalMissions
{
	BYTE	bHeader;
	BYTE	type;
	std::string	name;
} TPacketGGNewGlobalMissions;
#endif
...
typedef struct packet_offline_shop_start_p2p
{
	...
	packet_offline_shop_start_p2p() : owner_vid(0)
	{
...
#ifdef FATE_ROULETTE
typedef struct SPacketFateRoulette
{
	BYTE    bHeader;
} TPacketGGFateRoulette;
#endif
...
typedef struct SPacketCGBattlePassComplateMission
{
	BYTE bHeader;
	BYTE bType;
	BYTE	bBattlePassID;
	BYTE bMissionIndex;
} TPacketCGBattlePassComplateMission;
#endif
