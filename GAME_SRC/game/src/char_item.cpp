...
)
{
	...
	case EQUIPMENT:
		{
			...
			if (pOld)
			{
				if (wCell < EXINVENTORY_POS_END)
				{
					for (int i = 0; i < pOld->GetSize(); ++i)
					{
						int p = wCell + (i * 5);

						if (p >= EXINVENTORY_POS_END)
							continue;
	...
	if (pItem)
	{
		...
		case INVENTORY:
		case EQUIPMENT:
			if ((wCell < EXINVENTORY_POS_END) || (BELT_INVENTORY_SLOT_START <= wCell && BELT_INVENTORY_SLOT_END > wCell))
...
LPITEM CHARACTER::GetWear(BYTE bCell) const
{
	...
	return m_pointsInstant.pItems[EXINVENTORY_POS_END + bCell];
}
...
void CHARACTER::SetWear(BYTE bCell, LPITEM item)
{
	...
	SetItem(TItemPos (INVENTORY, EXINVENTORY_POS_END + bCell), item);
...
bool CHARACTER::IsEmptyItemGrid(TItemPos Cell, BYTE bSize, int iExceptionCell, LPITEM item) const
{
	...
	case INVENTORY:
		{
			const bool maxCell = (item && (exInven_IsExInvenItem(item)) != -1) ? EXINVENTORY_POS_END : Inventory_Size();
	...
	switch (Cell.window_type)
	{
		case INVENTORY:
		{
			...
			if (Cell.IsBeltInventoryPosition()) {
				LPITEM beltItem = GetWear(WEAR_BELT);
				if (nullptr == beltItem) return false;
				if (false == CBeltInventoryHelper::IsAvailableCell(bCell - BELT_INVENTORY_SLOT_START, beltItem->GetValue(0))) return false;
				if (m_pointsInstant.bItemGrid[bCell])
				//if (GetFromItemGrid(bCell))
				{
					if (m_pointsInstant.bItemGrid[bCell] == iExceptionCell) return true;
					//if (GetFromItemGrid(bCell) == iExceptionCell) return true;
					return false;
				}
				if (bSize == 1) return true;
			}
			else if (bCell >= EXINVENTORY_POS_END) return false;
			if (m_pointsInstant.bItemGrid[bCell])
			//if (GetFromItemGrid(bCell))
			{
				if (m_pointsInstant.bItemGrid[bCell] == iExceptionCell)
				//if (GetFromItemGrid(bCell) == iExceptionCell)
				{
					if (bSize == 1) return true;
					int32_t j = 1;
					BYTE bPage = bCell / (EXINVENTORY_POS_END / INVENTORY_PAGE_COUNT);
					do
					{
						DWORD p = bCell + (5 * j);
						if (p >= EXINVENTORY_POS_END) return false;
						if (p / (EXINVENTORY_POS_END / INVENTORY_PAGE_COUNT) != bPage) return false;
						if (m_pointsInstant.bItemGrid[p])
							if (m_pointsInstant.bItemGrid[p] != iExceptionCell)
			...
			if (1 == bSize) return true;
			{
				...
				{
					DWORD p = bCell + (5 * j);
					if (p >= INVENTORY_MAX_NUM) return false;
					if (p / (INVENTORY_MAX_NUM / INVENTORY_PAGE_COUNT) != bPage) return false;
					if (m_pointsInstant.bItemGrid[p])
						if (m_pointsInstant.bItemGrid[p] != iExceptionCell)
					//if (GetFromItemGrid(p))
						//if (GetFromItemGrid(p) != iExceptionCell)
...
int CHARACTER::CountEmptyInventory() const
{
	...
	for (int i = 0; i < Inventory_Size(); ++i)
#else
	for (int i = 0; i < EXINVENTORY_POS_END; ++i)	
#endif
...
#else
	return (EXINVENTORY_POS_END - count);
#endif
}
...
bool CHARACTER::DoRefine(LPITEM item, bool bMoneyOnly)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "Event haritasinda bu islemi yapamazsiniz.");
		return false;
	}
#endif
...
#ifdef ENABLE_NEW_MISSIONS
			const auto targetVnum = CNewMissions::instance().GetGlobalMissionValue(CNewMissions::eGlobalMissionType::FIRST_POISON_SWORD);
			if (pkNewItem->GetVnum() == targetVnum) {
				CNewMissions::instance().UpdateGlobalMissions(this, pkNewItem->GetVnum(), CNewMissions::eGlobalMissionType::FIRST_POISON_SWORD);
			}
#endif
...
#ifdef ENABLE_SOULBIND_SYSTEM
	if(item->IsSealed()){
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Can't upgrade sealbind item."));
		return false;
	}
#endif
...
{
	if (IsDead()) return;
	if (GetExchange() || IsCubeOpen() || IsOpenSafebox() || GetShop()) { ChatPacket(CHAT_TYPE_INFO, "Acik olan pencereleri kapat ve tekrar dene."); return; }
	if (GetMyShop() || GetOfflineShop()) { ChatPacket(CHAT_TYPE_INFO, "Zaten olan bir ?eyi yeniden olu?turmay©¥ denemiyor musun? Sorun ya?©¥yorsan Discord'a gel bir bakal©¥m."); return; }
...
bool CHARACTER::UseItemEx(LPITEM item, TItemPos DestCell)
{
	...
	switch (item->GetType())
	{
		...
		case ITEM_NEW_BLEND:
		{
			...
			{
				//value0=time via minute
				//value1=potion idx or if 0 will be add all potion
				case BLEND_TIME:
				{
					LPITEM item2 = GetItem(DestCell);
					if (item2)
					{
						if (!(item2->GetType() == ITEM_NEW_BLEND && item2->GetSubType() == BLEND_POTION))
						{
							ChatPacket(CHAT_TYPE_INFO, "You can't use on this item.");
							return false;
						}
						if (item2->GetSocket(0) == 1)
						{
							ChatPacket(CHAT_TYPE_INFO, "You can't add time on active dew.");
							return false;
						}
						if (item->GetValue(1) != 0 && item2->GetVnum() != item->GetValue(1))
						{
							ChatPacket(CHAT_TYPE_INFO, "You can't add time on this potion.");
							return false;
						}
						long newTime = item2->GetSocket(1);
						if (newTime < 0)
							newTime = 0;
						newTime += item->GetValue(0) * 60;

						item2->SetSocket(1, newTime);
						item->SetCount(item->GetCount() - 1);
						ChatPacket(CHAT_TYPE_INFO, "Successfuly time added.");
						return true;
					}
				}
				break;
				case BLEND_POTION:
				{
					...
					if (GetProtectTime("new_blend.last_click") > now)
					{
						ChatPacket(CHAT_TYPE_INFO, "You need wait some seconds.");
						return false;
					}
					SetProtectTime("new_blend.last_click", now + 1);

					if (FindAffect(AFFECT_BLEND, aApplyInfo[item->GetApplyType(0)].bPointType) || FindAffect(510, aApplyInfo[item->GetApplyType(0)].bPointType))
					{
					...
					if (!newStatus)
					{
						if (!item->GetSocket(1))
						{
							ChatPacket(CHAT_TYPE_INFO, "You need add time for you can use..");
							return false;
						}

						std::vector<CAffect*> m_vec;
						GetAffects(AFFECT_NEW_POTION, aApplyInfo[item->GetApplyType(0)].bPointType, m_vec);
						for(WORD i=0;i<m_vec.size();++i)
						{
							CAffect* affect = m_vec[i];
							LPITEM potionItem = FindItemByID(affect->dwFlag);
							if(potionItem && potionItem == item)
							{
								RemoveAffect(affect);
								break;
							}
						...
						//RemoveAffectNew(AFFECT_NEW_POTION, aApplyInfo[item->GetApplyType(0)].bPointType, item);

						//CAffect* affect = FindAffect(AFFECT_NEW_POTION, aApplyInfo[item->GetApplyType(0)].bPointType);
						//if (affect)
						//{
						//	LPITEM potionItem = FindItemByID(affect->dwFlag);
						//	if (potionItem && potionItem != item)
						//	{
						//		potionItem->SetSocket(0, 0);
						//		potionItem->SetSocket(1, (potionItem->GetSocket(1) - now) > 0 ? potionItem->GetSocket(1) - now : 0);
						//	}
						//	RemoveAffect(affect);
						//}
						
						const int leftTime = (item->GetSocket(1) - now) > 0 ? item->GetSocket(1) - now : 0;
						item->SetSocket(0, newStatus);
						item->SetSocket(1, leftTime);
					}
					else
					{
						const int lastTime = item->GetSocket(1);
						if (lastTime <= 0)
						{
							item->SetSocket(0, 0);
							item->SetSocket(1, 0);
							ChatPacket(CHAT_TYPE_INFO, "Not has time!");
							return false;
						}
						...
						for(WORD i=0;i<m_vec.size();++i)
						{
							...
							LPITEM potionItem = FindItemByID(affect->dwFlag);
							if(potionItem && potionItem->GetVnum() == item->GetVnum())
							{
						...
						AddAffect(AFFECT_NEW_POTION, aApplyInfo[item->GetApplyType(0)].bPointType, item->GetApplyValue(0), item->GetID(), lastTime, 0, false);
		...
		case ITEM_UNIQUE:
			{
				switch (item->GetSubType())
				{
					case USE_ABILITY_UP:
						{
#ifdef __NEW_POTION__
							if (item->GetValue(1) != 0 && FindAffect(AFFECT_NEW_POTION, aApplyInfo[item->GetValue(1)].bPointType))
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ÀÌ¹Ì È¿°ú°¡ °É·Á ÀÖ½À´Ï´Ù."));
								return false;
							}
		...
		case ITEM_PICK:
			...
			if (!item->IsEquipped())
				...
				{
					ChatPacket(CHAT_TYPE_INFO, "Bunu yapabilmek icin %d saniye bekleyin.", (GetQuestFlag("ARMOR.CHECKER") - get_global_time()) % 60);
					return false;
				}
				EquipItem(item);
		...
		case ITEM_GIFTBOX:
			{
				...
				if (GiveItemFromSpecialItemGroup(dwBoxVnum, dwVnums, dwCounts, item_gets, count))
				{
					...
					SetRankManuel(RANK_CHEST, 1);
#endif
#ifdef ENABLE_NEW_MISSIONS
					CNewMissions::instance().UpdateMissionData(this, CNewMissions::MISSION_CHEST);
#endif
					...
					case USE_POTION_NODELAY:
						{
							...
							if (item->GetVnum() == 50816) {

								const auto& mapList = { 5,25,66,107,108,109,208,216 };
								bool isBlock = false;
								...
								if (isBlock) {
									ChatPacket(1 ,"Bu esya sadece Boss Zindanlarinda kullanilabilir!");
									return false;
								}
				...
				switch (item->GetSubType())
				{
					...
					case USE_SPECIAL:
						...
						switch (item->GetVnum())
						{
							...
							case 99951:
							{
								if (FindAffect(AFFECT_THIRD_HAND))
								{
									ChatPacket(CHAT_TYPE_INFO, "Bu etki zaten devam ediyor.");
									return false;
								}
								AddAffect(AFFECT_THIRD_HAND, POINT_NONE, 0, AFF_NONE, 60*60*24*7, 0, true, true);
								item->SetCount(item->GetCount()-1);
								ChatPacket(CHAT_TYPE_INFO, "Etki devam etti? s?ece itemler do?udan envanterine gelecek.");
							}
							break;
#endif
							...
							case 99952:
							{
								if (FindAffect(AFFECT_ANTI_MOB))
								{
									ChatPacket(CHAT_TYPE_INFO, "Bu etki zaten devam ediyor.");
									return false;
								}
								AddAffect(AFFECT_ANTI_MOB, POINT_NONE, 0, AFF_NONE, 60*60*24*7, 0, true, true);
								item->SetCount(item->GetCount()-1);
								ChatPacket(CHAT_TYPE_INFO, "Etki devam etti? s?ece menzilli t? yarat?lar yan??a kadar gelecek.");
							}
							break;
#endif
							...
							case 90062:
							{
								const auto bioIdx = item->GetValue(0);
								if (CNewBiologManager::instance().EndBiolog(this, bioIdx, true))
								{
									item->SetCount(item->GetCount() - 1);
								}								
							}
					...
					case 10051:
					{
						if (!FindAffect(AFFECT_UNLIMITED_SKILL_SCROLL))
						{
							const bool isUnlimited = item->GetVnum() == 10050;
							const auto time = isUnlimited ? INFINITE_AFFECT_DURATION : 3 * 24 * 60 * 60;
							AddAffect(AFFECT_UNLIMITED_SKILL_SCROLL, POINT_NONE, 0, AFF_NONE, time, 0, false, true);
							ChatPacket(CHAT_TYPE_INFO, "You have %s skill scroll effect.", isUnlimited ? "unlimited" : "3 days");
							SetQuestFlag("unlimited_skill.status", 1);
							item->SetCount(item->GetCount() - 1);
						}
						else
						{
							ChatPacket(CHAT_TYPE_INFO, "Beceri persomeni zaten aktif.");
						}
					}
					break;

					case 10056: // Yeni eþyan? kodu
					{
    						if (!FindAffect(AFFECT_UNLIMITED_SKILL_SCROLL))
    						{
        						const auto time = 7 * 24 * 60 * 60; // 7 g? s?esi
        						AddAffect(AFFECT_UNLIMITED_SKILL_SCROLL, POINT_NONE, 0, AFF_NONE, time, 0, false, true);
        						ChatPacket(CHAT_TYPE_INFO, "You have 7 days skill scroll effect.");
        						SetQuestFlag("unlimited_skill.status", 1);
        						item->SetCount(item->GetCount() - 1);
    						}
    						else
...
//                            		case 10053:
//                            		{
//                                		CompleteNextExtBattlePassMission();
//                                		ChatPacket(1, "Battlepass mission successfully complated");
//                                		item->SetCount(item->GetCount() - 1);
//                            		}
//#endif
							...
							case 85850 ... 85855:
							{
								const auto mission_Type = item->GetValue(0);
								if (CNewMissions::instance().AddPlayer(this, mission_Type)) {
									item->SetCount(item->GetCount()-1);
								}
							}
							break;
							...
							case 50335: case 50336: case 50341: //SKILL_MONSTER_BONUS
							case 50337: case 50338: case 50342: //SKILL_BOSS_BONUS
							case 50339: case 50340: case 50343: //SKILL_BOSS_BONUS
#endif
								{
									if (IsPolymorphed() == true)
									{
...
#ifdef ENABLE_NEW_PASSIVE_SKILL
										case 50335: case 50336: case 50341:
											dwSkillVnum = SKILL_MONSTER_BONUS;
											break;
										case 50337: case 50338: case 50342:
											dwSkillVnum = SKILL_STONE_BONUS;
											break;
										case 50339: case 50340: case 50343:
						...
						case 30186: 
						{
						    ...
						    if ((item2->GetType() == ITEM_WEAPON) || (item2->GetType() == ITEM_ARMOR && item2->GetSubType() == ARMOR_BODY))
						    {
						        ...
						        item2->UpdatePacket();
						        item2->Save();
						        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("µåµð¾î ³¡ÀÌ º¸ÀÌ´Â °Ç°¡... ÀÌ ±â¼úÀº ÀÌÇØÇÏ±â°¡ ³Ê¹« Èûµé¾î.."));
						        item->SetCount(item->GetCount() - 1);
						    }
						    else
						    {
						...
						case 92226: 
						{
						    ...
						    if ((item2->GetType() == ITEM_WEAPON) || (item2->GetType() == ITEM_ARMOR && item2->GetSubType() == ARMOR_BODY))
						    {
						        if (item2->GetSocketCount() == ITEM_SOCKET_MAX_NUM)
						        {
						            ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Yeterli yer yok."));
						            return false;
						        }
						        item2->AddSocket();
						        item2->UpdatePacket();
						        item2->Save();
						        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Yeni slot eklendi."));
						        item->SetCount(item->GetCount() - 1);
						    }
						    else
						    {
						        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu esyaya yeni bir slot ekleyemezsin."));
						        break;
						    }
						}
						break;
						...
						case 92227:
						{
						    ...
						    if ((item2->GetType() == ITEM_WEAPON))
						    {
						        if (item2->GetSocketCount() == ITEM_SOCKET_MAX_NUM)
						        {
						            ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Silahinizin maksimum tas slotu zaten dolu."));
						            return false;
						        }
						        item2->AddSocket();
						        item2->UpdatePacket();
						        item2->Save();
						        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Silahiniza 4.Tas Slotu basariyla eklendi."));
						        item->SetCount(item->GetCount() - 1);
						    }
						    else
						    {
						        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Uzgunum, bu esyaya bir tas slotu ekleyemezsin.."));
						        break;
						    }
						}
						break;
						...
						case 92228:
						{
						    ...
						    if ((item2->GetType() == ITEM_ARMOR && item2->GetSubType() == ARMOR_BODY))
						    {
						        if (item2->GetSocketCount() == ITEM_SOCKET_MAX_NUM)
						        {
						            ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Zirhinizin maksimum tas slotu zaten dolu."));
						            return false;
						        }
						        item2->AddSocket();
						        item2->UpdatePacket();
						        item2->Save();
						        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Zirhiniza 4.Tas Slotu basariyla eklendi."));
						        item->SetCount(item->GetCount() - 1);
						    }
						    else
						    {
		...
		case ITEM_BLEND:
			...
			if (Blend_Item_find(item->GetVnum()))
			{
...
#ifdef __NEW_POTION__
				if (apply_type != 0 && FindAffect(AFFECT_NEW_POTION, apply_type))
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu etkiye zaten sahipsin"));
					return false;
				}
#endif
				...
				if (FindAffect(affect_type, apply_type))
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Sebnem etkisi devam ediyor."));
				}
				else
				{
...
#ifdef __OFFLINE_PRIVATE_SHOP_SYSTEM__
		if (HasOfflineShop()) {
			ChatPacket(CHAT_TYPE_INFO, "Zaten olan bir seyi yeniden olu?turmay©¥ denemiyor musun? Sorun ya?©¥yorsan Discord'a gel bir bakal©¥m.");
			return false;
		}
#endif
	}
...
bool CHARACTER::DeleteItem(TItemPos Cell)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Lutfen bir sure bekledikten sonra tekrar deneyin."));
		return false;
	}

	if (GetExchange() || GetMyShop() || GetShopOwner() || IsOpenSafebox() || IsCubeOpen())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Baska bir pencere acik olmadigindan emin olun."));
		return false;
	}
	// EXTRA_CHECK
...
#endif
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ITEM_BASARIYLA_SILINDI"));
	ITEM_MANAGER::instance().RemoveItem(item);
...
{
	//ChatPacket(1, "cell : %d - dest : %d", Cell.cell, DestCell.cell);
	LPITEM item = NULL;
	...
	if (INVENTORY == Cell.window_type && Cell.cell >= EXINVENTORY_POS_END && IS_SET(item->GetFlag(), ITEM_FLAG_IRREMOVABLE))
		return false;
	...
	if (DestCell.IsEquipPosition())
	{
		...
		EquipItem(item, DestCell.cell - EXINVENTORY_POS_END);
	}
	else
	{
...
bool CHARACTER::PickupItem(DWORD dwVID)
{
	...
	if (item->DistanceValid(this))
	{
		...
		if (item->IsOwnership(this))
		{
			...
			if (IsAutomaticItemProcess(item->GetVnum()) && GetQuestFlag("automatic_item_process.status") == 1)
			{
				const auto price = item->GetShopBuyPrice() * item->GetCount();
				GiveGold(price);
			...
			else
			{
				...
				if (item->IsDragonSoul())
					...
					if (!exInven_PutOn(item, true)) item->AddToCharacter(this, TItemPos(INVENTORY, iEmptyCell));
		...
		else if (!IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_DROP) && GetParty())
		{
			...
			if (item->IsDragonSoul())
				...
				if (!exInven_PutOn(item, true)) item->AddToCharacter(owner, TItemPos(INVENTORY, iEmptyCell));
...
bool CHARACTER::SwapItem(BYTE bCell, WORD bDestCell)
{
	if (!CanHandleItem())
		return false;
	...
	{
		BYTE bEquipCell = item2->GetCell() - EXINVENTORY_POS_END;
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	if (item->IsDragonSoul())
	{
		...
		if(GetInventoryItem(EXINVENTORY_POS_END + iWearCell))
		{
	...
	else
	{
		...
		if (GetWear(iWearCell) && !IS_SET(GetWear(iWearCell)->GetFlag(), ITEM_FLAG_IRREMOVABLE))
		{
			...
			if (false == SwapItem(item->GetCell(), EXINVENTORY_POS_END + iWearCell))
			{
				return false;
			}
		}
...
#else
	for (UINT i = 0; i < EXINVENTORY_POS_END; ++i)
#endif
	{
...
void CHARACTER::AutoGiveItem(LPITEM item, bool longOwnerShip)
{
	...
	else if ((exInven_IsExInvenItem(item)) != -1)
	{
		cell = exInven_FindPos(item);
	}
	else
	{
	...
	if (cell != -1)
	{
		if (item->IsDragonSoul())
			...
			if (!exInven_PutOn(item, true)) item->AddToCharacter(this, TItemPos(INVENTORY, cell));
...
LPITEM CHARACTER::AutoGiveItem(DWORD dwItemVnum, BYTE bCount, int iRarePct, bool bMsg)
{
	...
	{
		iEmptyCell = exInven_FindPos(item);
	}
	else
	...
	if (iEmptyCell != -1)
	{
		...
		if (item->IsDragonSoul())
			...
			if (!exInven_PutOn(item, bMsg)) item->AddToCharacter(this, TItemPos(INVENTORY, iEmptyCell));
		LogManager::instance().ItemLog(this, item, "SYSTEM", item->GetName());
