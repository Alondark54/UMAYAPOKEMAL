...
//	}
	...
	ALUA(pc_have_map_scroll)
	{
		...
		bool bFind = false;
		for (int iCell = 0; iCell < EXINVENTORY_POS_END; iCell++)
		{
...
teleport_area:
	...
	ALUA(pc_get_socket_items)
	{
		...
		for ( int i=0; i < EXINVENTORY_POS_END + WEAR_MAX_NUM; i++ )
		{
	...
	int pc_update_mining_ranking(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		if (!lua_isnumber(L, 1))
		{
			sys_err("QUEST pc_update_mining_ranking : wrong argument");
			lua_pushnumber (L, 0);
			return 1;
		}
		...
		if (points < 1)
		{
			sys_err("QUEST pc_update_mining_ranking : points < 1");
			lua_pushnumber (L, 0);
			return 1;
		}

		std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT pid FROM %s WHERE pid = '%d'", "player.mining_ranking", ch->GetPlayerID()));
		
		if (pMsg->Get()->uiNumRows > 0)
		{
			DBManager::instance().DirectQuery("UPDATE %s SET total_points = total_points + '%d' WHERE pid = '%d'", "player.mining_ranking", points, ch->GetPlayerID());
		}
		else
		{
			DBManager::Instance().DirectQuery("INSERT INTO %s (pid, name, total_points) VALUES('%d', '%s', '%d')", "player.mining_ranking", ch->GetPlayerID(), ch->GetName(), points);	
		}
...
#ifdef ENABLE_CAOS_EVENT
	int pc_warp_caosevent(lua_State* L)
	{
		auto ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch) { return 0; }
		if (ch->GetLevel() < MIN_LEVEL_CAOS_EVENT)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Yeterli seviyede degilsin!");
			return 0;
		}
		CNewCaosEventManager::WarpPlayer(ch);
		return 1;
	}

	int pc_is_caoseventisopen(lua_State* L)
	{
		const bool isOpen = int(quest::CQuestManager::instance().GetEventFlag("caos_event")) == 2;
...
#ifdef ENABLE_MINING_EVENT
			{ "update_mining_ranking",	pc_update_mining_ranking	},
#endif
...
#endif
#ifdef ENABLE_CAOS_EVENT
			{"warp_caoseventmap", pc_warp_caosevent },
			{"is_caoseventopen", pc_is_caoseventisopen },
#endif
