...
bool CExchange::Done()
{
	...
	for (i = 0; i < EXCHANGE_ITEM_MAX_NUM; ++i)
	{
		...
		else if ((victim->exInven_IsExInvenItem(item)) != -1)
			empty_pos = victim->exInven_FindPos(item);
		else
		...
		if (item->IsDragonSoul())
			...
			if (!victim->exInven_PutOn(item, true)) item->AddToCharacter(victim, TItemPos(INVENTORY, empty_pos));
