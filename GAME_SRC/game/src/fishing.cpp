...
void Initialize()
{
	...
	for (int j = 0; j < MAX_PROB; ++j)
	{
		...
		const std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT * FROM `player`.`fishevent_items`"));

		if (pMsg->Get()->uiNumRows==0) {
			sys_err("Fish event table is empty!");
			return;
		}
		MYSQL_ROW row;
		fish_vec.clear();
...
void FishingPractice(LPCHARACTER ch)
{
	...
	if (rod && rod->GetType() == ITEM_ROD)
	{
		...
		if ( rod->GetRefinedVnum()>0 && rod->GetSocket(0) < rod->GetValue(2) && number(1,rod->GetValue(1))==1 )
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("???? ???? ???????! (%d/%d)"),rod->GetSocket(0), rod->GetValue(2));
...
#ifdef ENABLE_FISH_EVENT
			if(int(quest::CQuestManager::instance().GetEventFlag("balik_event")) == 1)
			{

				const BYTE random = number(0,fish_vec.size() - 1);
				const BYTE fishProb = fish_vec[random].bChance;
				const DWORD itemVnum = fish_vec[random].dwGiftVnum;
				info->fish_id = itemVnum;
				ch->SetFishChance(fishProb);
			}
			else
			{
...
#endif	
{
...
#ifdef ENABLE_FISH_EVENT
	if (!ch) { return -99; }
	if(int(quest::CQuestManager::instance().GetEventFlag("balik_event")) == 0)
	{
		if (fish_id >= MAX_FISH)
		{
	...
	if(int(quest::CQuestManager::instance().GetEventFlag("balik_event")) == 1)
	{
		...
		LPITEM rod = ch->GetWear(WEAR_WEAPON);// olta +sina gore sans arttirma

		if (rod && rod->GetType() == ITEM_ROD)
		{
			prob += (rod->GetValue(0) / 10);
		}

		if (prob < ch->GetFishChance())
		{
			*item = fish_id;
			return 0;
		}
			...
			case 0:
				if (item_vnum)
				{
					...
					if (item)
					{
						...
						if (test_server)
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("??? ?? ???? ??? %.2fcm"), item->GetSocket(0)/100.f);
						}
						...
						if (quest::CQuestManager::instance().GetEventFlag("fishevent") > 0 && (info->fish_id == 5 || info->fish_id == 6))
						{
							...
							if (info->fish_id == 5)
							{
								strlcpy(p.szBoard, LC_TEXT("?????????"), sizeof(p.szBoard));
							}
...
void Grill(LPCHARACTER ch, LPITEM item)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s? ?????."), item->GetName());
	item->SetCount(0);
