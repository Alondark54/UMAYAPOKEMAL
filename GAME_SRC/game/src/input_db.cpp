...
#ifdef __OFFLINE_PRIVATE_SHOP_SYSTEM__
	MYSQL_ROW row;
	auto pMsg(DBManager::instance().DirectQuery("SELECT * FROM %soffline_shop_npc", get_table_postfix()));
	if (pMsg->Get()->uiNumRows != 0) {
		char szName[CHARACTER_NAME_MAX_LEN + 1];
		char szSign[SHOP_SIGN_MAX_LEN + 1];
		DWORD dwOwnerID = 0;
		int32_t lMapIndex = 0, x = 0, y = 0, z = 0;
		int32_t iTime = 0, npc_vnum = 0;
		uint8_t sqlCur = 0, border_type = 0, bChannel = 0;
		while (nullptr != (row = mysql_fetch_row(pMsg->Get()->pSQLResult))) {
			sqlCur = 0;

			str_to_number(dwOwnerID, row[sqlCur++]);
			snprintf(szSign, sizeof(szSign), "%s", row[sqlCur++]);
			snprintf(szName, sizeof(szName), "%s", row[sqlCur++]);
			str_to_number(iTime, row[sqlCur++]);
			str_to_number(x, row[sqlCur++]);
			str_to_number(y, row[sqlCur++]);
			str_to_number(z, row[sqlCur++]);
			str_to_number(lMapIndex, row[sqlCur++]);
			str_to_number(bChannel, row[sqlCur++]);
			str_to_number(npc_vnum, row[sqlCur++]);
			str_to_number(border_type, row[sqlCur++]);
			...
			if (g_bChannel == bChannel) {
				...
				if (npc) {
					...
					if (iTime != 0) { npc->SetOfflineShopTimer(iTime); npc->StartOfflineShopUpdateEvent(); }
					LPOFFLINESHOP pkOfflineShop = COfflineShopManager::instance().CreateOfflineShop(npc, dwOwnerID, nullptr, 0, lMapIndex, iTime, szSign, border_type);
...
#endif
#ifdef ENABLE_LUCKY_DRAW
        case HEADER_DG_LUCKY_DRAW_ADD_PARTICIPANT:
            LuckyDraw::instance().RESULT_ADD_PARTICIPANT((TPacketDGLuckyDrawAddParticipant*)c_pData);
            break;
        case HEADER_DG_LUCKY_DRAW_CURRENT_LD:
            LuckyDraw::instance().RESULT_LUCKYDRAW_TABLE((TLuckyDrawTable*)c_pData);
            break;
#endif
