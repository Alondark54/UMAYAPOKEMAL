...
ACMD(do_item)
{
	...
	if (item)
	{
		...
		{
			int32_t iEmptyPos = ch->exInven_IsExInvenItem(item) != -1 ? ch->exInven_FindPos(item) : ch->GetEmptyInventory(item->GetSize());
...
ACMD(do_item_purge)
{
	...
	{
		for (i = 0; i < EXINVENTORY_POS_END; ++i)
		{
			if ((item = ch->GetInventoryItem(i)))
			{
				ITEM_MANAGER::instance().RemoveItem(item, "PURGE");
	...
	else if (!strArg.compare(0, 5, "equip"))
	{
		...
		{
			if ((item = ch->GetInventoryItem(EXINVENTORY_POS_END + i)))
			{
				ITEM_MANAGER::instance().RemoveItem(item, "PURGE");
	...
	if (*arg1)
	{
		switch (LOWER(*arg1))
		{
			...
			case 'z':	
				...
				ch->ChatPacket(CHAT_TYPE_INFO, "Reloaded Roulette! Roulette size %d",CFateRoulette::instance().ItemSize());
				TPacketGGFateRoulette pggFateRoulette;
				pggFateRoulette.bHeader = HEADER_GG_FATE_ROULLETTE;
				P2P_MANAGER::instance().Send(&pggFateRoulette, sizeof(TPacketGGFateRoulette));
				break;
#endif
	...
	case JOB_SHAMAN:
		{
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(7199 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
	...
	case JOB_ASSASSIN:
		{
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(4049 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
...
const char* empireflags[3] = { "|cFFFF0000|H|h[PM]|h|r","|cFFFF0000|H|h[PM]|h|r","|cFFFF0000|H|h[PM]|h|r" };

const char* komutArray_1[] = { //Normal Selamlar
	"as","As","AS","Aleykum Selam", "a.s.", "A.S", "ASS"
};
...
const char* komutArray_3[] = { //Random Konusmalar
	"Dolu, kutsama satilir pm","Erik yiyorum","Elma yiyorum","Yanan hayallerdi, siz lastikleri gordunuz","BK SATILIR TOPLU","TOPLU HAVA BK ALINIR","TRIBUNDE GUVENCE UNIVERSITE 89","BEN KEMAL GELIYORUM","Dolu KDP Pembe Sort ALINIR","Beni dinle adanaliyyyyy","Hava cok sicak aq","Yeter lan bu ne sicak","olm server harika lan","abi sw super","nerde level kascaz aq","LEGAL YANG SATILIR @@@@@@","EFSUN MUNZEVI SATILIR","ZEN ALINMAZ @@@@@@@@","KIRMIZI DEMIR PALA ALINIR"
};
const char* karakterAdArray[] = {
	...
	"mezarci","Mira","Poncik2","amra","erkumenkozer","ergumenkozer","BASBELASI","SIZOFREN","poncik3","Resque","Strawberry","ponc","OCBerkinElvn", "HikmetAypek","SCA", "Tunga", "Bekomango", "Tek7","Dreamfancy","Luigina","Orhanbaskan","Sado45","ZiyadMisali","CBaran","DeRiNDaRBe","YusufBaran","BeViS","LeLaXi","BonusGamer"
};
...
{
	#define numberberkay(from, to) (rand() % to + from)
	...
	if (*iSure > 0) {
		...
		BYTE karakterBayrak = 0;
		karakterAd = karakterAdArray[numberberkay(0, (sizeof(karakterAdArray)/sizeof(*karakterAdArray)) - 1)];
		karakterBayrak = numberberkay(0,2);
		//int karakterLevel = numberberkay(1, 99);

		if (info->bKomutTipi == 1) { karakterMesaj = komutArray_1[rand() % (sizeof(komutArray_1)/sizeof(*komutArray_1) - 1)]; }//Normal Selam.
		if (info->bKomutTipi == 2) { karakterMesaj = komutArray_2[rand() % (sizeof(komutArray_2)/sizeof(*komutArray_2) - 1)]; } //Karisik Selam.
		if (info->bKomutTipi == 3) { karakterMesaj = komutArray_3[rand() % (sizeof(komutArray_3)/sizeof(*komutArray_3) - 1)]; } //Random Konusmalar.
		char szNotice[512 + 1];
		...
		snprintf(szNotice, sizeof(szNotice), "%s %s |H%s%s|h|h|r : %s", karakterAd, empireflags[karakterBayrak], "whisper:", karakterAd, karakterMesaj);
...
ACMD(do_chatBot)
{
	...
	one_argument(two_arguments(argument, arg1, sizeof(arg1), arg2, sizeof(arg2)), arg3, sizeof(arg3));
	if (!*arg1 || !*arg2 || !*arg3) {
		ch->ChatPacket(CHAT_TYPE_INFO, "-------Kullanim: /chatbot <komut tipi> <bot suresi (saniye)> <bot gecis suresi (saniye)>-------");
...
ACMD(do_offline_shop_sales)
{
	...
	if (pMsg->Get()->uiNumRows > 0) {
		for (uint8_t i = 0; i < pMsg->Get()->uiNumRows; ++i) {
