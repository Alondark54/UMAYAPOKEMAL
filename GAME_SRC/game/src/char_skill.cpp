...
void CHARACTER::SkillLevelPacket()
{
	...
	if (GetMapIndex() == CAOS_EVENT_MAP) {
		TPlayerSkill tempSkillLevels[SKILL_MAX_NUM]{};
		for (BYTE i(0); i<SKILL_MAX_NUM; i++) {
			if (CanUseSkill(i))
			{
				tempSkillLevels[i].bMasterType = SKILL_PERFECT_MASTER;
				tempSkillLevels[i].bLevel = SKILL_MAX_LEVEL;
			}
		}
		thecore_memcpy(&pack.skills, tempSkillLevels, sizeof(TPlayerSkill) * SKILL_MAX_NUM);
	}
	else {
