...
bool __CHARACTER_GotoNearTarget(LPCHARACTER self, LPCHARACTER victim)
{
	...
	float range = 1.0;
	if (victim->GetAntiMobRange() == false)
	{
		range = self->GetMobAttackRange();
	}
#endif
...
#ifdef ANTI_MOB_RANGE_ITEM	
			if (self->Follow(victim, range * 8 / 10))
#else
...
#ifdef ANTI_MOB_RANGE_ITEM	
			if (self->Follow(victim, range * 9 / 10))
#else
