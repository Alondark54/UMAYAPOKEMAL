...
#define IS_NO_SAVE_AFFECT(type) ((type) == AFFECT_AUTO_HUNT_AFFECT || (type) == AFFECT_WAR_FLAG || (type) == AFFECT_REVIVE_INVISIBLE || ((type) >= AFFECT_PREMIUM_START && (type) <= AFFECT_PREMIUM_END) || (type) == AFFECT_MOUNT_BONUS) // @fixme156 added MOUNT_BONUS (if the game core crashes, the bonus would double if present in player.affect)
...
DWORD GetNewPotionItemIdx(LPCHARACTER ch, const CAffect* pkAff)
{
	if(pkAff->dwType == AFFECT_NEW_POTION)
	{
		LPITEM potionItem = ch->FindItemByID(pkAff->dwFlag);
		return potionItem ? potionItem->GetVnum() - 50770 : pkAff->bApplyOn;
	}
	return pkAff->bApplyOn;
}
#endif
...
#ifdef __NEW_POTION__
	ptoc.elem.bApplyOn		= GetNewPotionItemIdx(d->GetCharacter(), pkAff);
#else
...
#ifdef __NEW_POTION__
void CHARACTER::GetAffects(DWORD dwType, BYTE bApply, std::vector<CAffect*>& m_vec)
{
	itertype(m_list_pkAffect) it = m_list_pkAffect.begin();
	while (it != m_list_pkAffect.end())
	{
		CAffect * pkAff = *it++;
		if(pkAff->dwType == dwType && pkAff->bApplyOn == bApply)
			m_vec.emplace_back(pkAff);
	}
}
void CHARACTER::RemoveAffectNew(DWORD dwType, BYTE bApply, LPITEM item)
{
	itertype(m_list_pkAffect) it = m_list_pkAffect.begin();
	while (it != m_list_pkAffect.end())
	{
		...
		if(pkAff->dwType == dwType && pkAff->bApplyOn == bApply)
		{
			LPITEM potionItem = FindItemByID(pkAff->dwFlag);
			if(potionItem == item)
			{
				RemoveAffect(pkAff);
				return;
...
bool CHARACTER::UpdateAffect()
{
	...
	if (!FindAffect(AFFECT_UNLIMITED_SKILL_SCROLL) && GetQuestFlag("unlimited_skill.status") == 1 && (IsAffectFlag(AFF_GEOMGYEONG) ||  IsAffectFlag(AFF_GWIGUM)))
	{
		...
		SetQuestFlag("unlimited_skill.status", 0);
	}
#endif
	...
	if (!GetWear(WEAR_WEAPON))
    	{
        	if (IsAffectFlag(AFF_GEOMGYEONG))
        	{
            		ChatPacket(CHAT_TYPE_INFO, LC_TEXT ("Hava kýlýcý skilini açmak için ilk önce silahýnýzýn takýlý olmasý gerekiyor. "));
            		RemoveAffect(SKILL_GEOMKYUNG);
        	}
    
        	if (IsAffectFlag(AFF_GWIGUM))
        	{
            		ChatPacket(CHAT_TYPE_INFO, LC_TEXT ("Büyülü silah skilini açmak için ilk önce silahýnýzýn takýlý olmasý gerekiyor."));
            		RemoveAffect(SKILL_GWIGEOM);
        	}
    	}
...
#endif
{
	...
	while (it != m_list_pkAffect.end())
	{
		...
		if (bSave)
		{
			...
			if(pkAff->dwType==AFFECT_DBONE_1 || pkAff->dwType==AFFECT_DBONE_2){/**/++it;/**/continue;}
#endif

			if(pkAff->dwType==AFFECT_YELLOW || pkAff->dwType==AFFECT_BROWN || pkAff->dwType==AFFECT_GREY
			|| pkAff->dwType==AFFECT_ORANGE || pkAff->dwType==AFFECT_PINK || pkAff->dwType==AFFECT_PURPLE || pkAff->dwType==AFFECT_TURQUOISE
			|| pkAff->dwType==AFFECT_BLUE || pkAff->dwType==AFFECT_RED){/**/++it;/**/continue;}
...
#ifdef THIRD_HAND
			if (pkAff->dwType == AFFECT_THIRD_HAND)
			{
				++it;
				continue;
			}
			...
			if (pkAff->dwType == AFFECT_ANTI_MOB)
			{
				++it;
				continue;
			}
...
#ifdef ENABLE_UNLIMITED_SKILL_SCROLL
			if (pkAff->dwType == AFFECT_UNLIMITED_SKILL_SCROLL)
			{
				++it;
				continue;
			}
			...
			const auto affectSize = GetQuestFlag("bp.affect");
			if (pkAff->dwType >= AFFECT_BATTLEPASSS_AFFECT_IDX && pkAff->dwType <= AFFECT_BATTLEPASSS_AFFECT_IDX + affectSize)
			{
				++it;
				continue;
			}
			...
			{
#ifdef __NEW_POTION__
				SendAffectRemovePacket(GetDesc(), GetPlayerID(), pkAff->dwType, GetNewPotionItemIdx(this, pkAff));
#else
...
#ifdef ANTI_MOB_RANGE_ITEM
	pkAff = FindAffect(AFFECT_ANTI_MOB);
	if (pkAff)
	{
		this->SetAntiMobRange(true);
	}
	else
	{
		this->SetAntiMobRange(false);
	}
#endif
...
bool CHARACTER::AddAffect(DWORD dwType, BYTE bApplyOn, long lApplyValue, DWORD dwFlag, long lDuration, long lSPCost, bool bOverride, bool IsCube )
{
	...
	const auto affectSize = GetQuestFlag("bp.affect");
	if (dwType >= AFFECT_BATTLEPASSS_AFFECT_IDX + affectSize) {
		SetQuestFlag("bp.affect", GetQuestFlag("bp.affect") + 1);
	}
	...
	switch (dwType)
	{
		...
		case SKILL_JEUNGRYEOK:
		{
			if (FindAffect(AFFECT_UNLIMITED_SKILL_SCROLL)) {
				lDuration = INFINITE_AFFECT_DURATION;
			}
		}
		break;
	}
...
#ifdef __NEW_POTION__
void CHARACTER::CheckNewBlend(CAffect* pkAff)
{
	if (pkAff)
	{
		LPITEM potionItem = FindItemByID(pkAff->dwFlag);
	...
	else
	{
		...
		{
			CAffect* pkAffect = *it++;
			if (pkAffect->dwType == AFFECT_NEW_POTION)
			{
				LPITEM potionItem = FindItemByID(pkAffect->dwFlag);
				if (potionItem && potionItem->GetSocket(0))
					potionItem->SetSocket(1, pkAffect->lDuration + time(0));
				else
				{
