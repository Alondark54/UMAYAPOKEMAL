...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	if (ch->GetQuestFlag("give_basic_weapon.source") != 1)
	{
		interpret_command(ch, "basic_item_old", 10);
	}
#endif
	...
	if (ch->GetMapIndex() == CAOS_EVENT_MAP) {
		...
		CNewCaosEventManager::instance().RegisterPlayer(ch);
	}
#endif
...
#ifdef FATE_ROULETTE
	if (CFateRoulette::instance().ItemSize() >= 16 && quest::CQuestManager::instance().GetEventFlag("fate_enable") == 1)
		ch->ChatPacket(CHAT_TYPE_COMMAND, "RouletteShowIcon");
#endif
