...
void CHARACTER::Reward(bool bItemDrop)
{
	...
	if (ITEM_MANAGER::instance().CreateDropItem(this, pkAttacker, s_vec_item))
	{
		...
		else if (s_vec_item.size() == 1)
		{
			...
			bool auto_loot = pkAttacker->FindAffect(AFFECT_THIRD_HAND);
			
			if (auto_loot)
			{
				pkAttacker->AutoGiveItem(item->GetVnum(), item->GetCount());
				sys_log(0, "THIRD_HAND: %s from %s", item->GetName(), GetName());
			}
			else
			{
		...
		else
		{
			...
			else
			{
				...
				while (iItemIdx >= 0)
				{
					...
					LPCHARACTER ch = *it;
					bool auto_loot = ch->FindAffect(AFFECT_THIRD_HAND);
					...
					if (auto_loot)
					{
						...
						sys_log(0, "THIRD_HAND: %s by %s", item->GetName(), GetName());
					}
					else
					{
						item->AddToGround(GetMapIndex(), pos);
	...
	if (bDropInventory) // Drop Inventory
	{
		...
		if (!vec_bSlots.empty())
		{
			...
			for (i = 0; i < iQty; ++i)
			{
				...
				if (pkItem->IsSealed())
				{
					ChatPacket(CHAT_TYPE_INFO, "Ruh korumasinda olan esyanin dusmesi engellendi. ");
					return;
				}
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (pkKiller && pkKiller->IsPC())
	{
		...
		{
			pkKiller->SetRankManuel(RANK_STONE, 1);
		}
		else if(IsMonster())
		{
			if (GetMobRank() >= MOB_RANK_BOSS)
				pkKiller->SetRankManuel(RANK_BOSS, 1);
			else
				pkKiller->SetRankManuel(RANK_MONSTER, 1);
		}
#endif


#ifdef ENABLE_CAOS_EVENT
		if (IsPC() && GetMapIndex() == CAOS_EVENT_MAP) {
			CNewCaosEventManager::instance().DeathPlayer(pkKiller->GetPlayerID(), GetPlayerID());
		}
#endif
#ifdef ENABLE_NEW_MISSIONS
		if (!IsPC() && GetMobRank() >= MOB_RANK_BOSS) {
			CNewMissions::instance().UpdateMissionData(pkKiller, CNewMissions::MISSION_KILL);
		}
#endif
	}
	...
	if (!IsPC() && pkKiller && pkKiller->IsPC())
	{
		...
		if (GetRaceNum() == 1192 && GetMapIndex() == 72)  // Guclu Buz Cadisi ( 4 Saat )
		{
			if (g_bChannel == 1)
		...
		if (GetRaceNum() == 2091 && GetMapIndex() == 104)  // Kralice Orumcek ( 1 Saat )
		{
			if (g_bChannel == 1)
		...
		if (GetRaceNum() == 2191 && GetMapIndex() == 63)  // Guclu Buz Cadisi ( 4 Saat )
		{
			if (g_bChannel == 1)
		...
		if (GetRaceNum() == 2307 && GetMapIndex() == 68)  // Guclu Buz Cadisi ( 4 Saat )
		{
			if (g_bChannel == 1)
		...
		if (GetRaceNum() == 2492 && GetMapIndex() == 73)  // Guclu Buz Cadisi ( 4 Saat )
		{
			if (g_bChannel == 1)
		...
		if (GetRaceNum() == 792 && GetMapIndex() == 65)  // Guclu Buz Cadisi ( 4 Saat )
		{
			if (g_bChannel == 1)
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	if ( pAttacker && pAttacker->IsPC()) {
			if (pAttacker->GetBotControlGUI()) { return false; }
			auto& botInstance = CNewBotControl::instance();
			if (IsStone() && CNewBotControl::instance().IsCheckTime(pAttacker)) { // -> just stone! if u have different type add this statement
					botInstance.CheckBotControl(pAttacker);
					return false;
			}
		}
#endif
...
#ifdef ENABLE_CAOS_EVENT
	if (pAttacker && pAttacker->IsPC() && IsPC() && GetMapIndex() == CAOS_EVENT_MAP)
	{
		const auto& [skillDmg, normalDmg] = CNewCaosEventManager::GetDamages(pAttacker->GetJob());
		if (skillDmg > 0 && normalDmg > 0) {
			switch (type)
			{
			case DAMAGE_TYPE_NORMAL:
			case DAMAGE_TYPE_NORMAL_RANGE:
...
#ifdef ENABLE_EXTENDED_BATTLE_PASS
	if (type != DAMAGE_TYPE_POISON && pAttacker != NULL)
	{
		if (IsPC())
