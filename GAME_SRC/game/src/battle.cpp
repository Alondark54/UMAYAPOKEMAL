...
#ifdef ENABLE_CAOS_EVENT
	if (ch->GetMapIndex() == CAOS_EVENT_MAP && (quest::CQuestManager::instance().GetEventFlag("caos_event") == 0 || quest::CQuestManager::instance().GetEventFlag("caos_event") == 2 ))
	{
		ch->ChatPacket(1, "Su an bu haritada savas yapamazsiniz.");
		return false;
	}
#endif
...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk)
{
	...
	if (pkVictim->IsNPC())
	{
		...
		if (pkVictim->IsRaceFlag(RACE_FLAG_ANIMAL))
			...
			iAtk += (iAtk * pkAttacker->GetPoint(POINT_RESIST_WOLFMAN)) / 100;
