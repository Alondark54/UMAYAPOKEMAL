...
void P2P_MANAGER::Boot(LPDESC d)
{
	...
	while (it != map.end())
	{
		...
		strlcpy(p.bIpAdress, ch->GetDesc()->GetHostName(), sizeof(p.bIpAdress));
		p.bMultiFarmStatus = ch->GetRewardStatus();
#endif
		d->Packet(&p, sizeof(p));
	}
}
...
#ifdef __MULTI_FARM_BLOCK__
	if (strlen(p->bIpAdress))
		CHARACTER_MANAGER::Instance().CheckMultiFarmAccount(p->bIpAdress, p->dwPID, p->szName, p->bMultiFarmStatus, false);
#endif
