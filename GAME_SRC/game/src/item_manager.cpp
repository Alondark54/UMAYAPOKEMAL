...
#ifdef ENABLE_BOT_CONTROL
		if ((m_vec_prototype[i].bType == ITEM_WEAPON || m_vec_prototype[i].bType == ITEM_ARMOR) && m_vec_prototype[i].dwVnum % 10 == 0) {
			CNewBotControl::instance().SetControlItems(m_vec_prototype[i].dwVnum, m_vec_prototype[i].szLocaleName);
		}
#endif
...
LPITEM ITEM_MANAGER::CreateItem(DWORD vnum, DWORD count, DWORD id, bool bTryMagic, int iRarePct, bool bSkipSave)
{
	...
	else
	{
		...
		if (item->GetType() == ITEM_NEW_BLEND && item->GetSubType() == BLEND_POTION)
		{
			//sys_err("blend item get time");
...
void ITEM_MANAGER::SaveSingleItem(LPITEM item)
{
	...
	{
		case EQUIPMENT:
			t.pos = item->GetCell() - EXINVENTORY_POS_END;
			break;
...
#ifdef ENABLE_DROP_FROM_TABLE
	MOB_DROP_MANAGER::instance().MakeDropInfoItems(pkChr, pkKiller, vec_item);
#endif
...
#ifdef ENABLE_DROP_FROM_TABLE
	MOB_DROP_MANAGER::instance().MakeDropItems(pkChr, pkKiller, vec_item);
#endif
...
void ITEM_MANAGER::CreateQuestDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM> & vec_item, int iDeltaPercent, int iRandRange)
{
	...
	if (GetDropPerKillPct(100, 2000, iDeltaPercent, "cards_drop") >= number(1, iRandRange))
	{
		sys_log(0, "À°°¢º¸ÇÕ DROP EVENT ");

		const static DWORD dwVnum = 79505;
