...
void CInputAuth::Login(LPDESC d, const char * c_pData)
{
	...
	// Lütfen keylerinizi değiştirmeyin ve paylaşmayınız. 
	// Paket bağlantısı mevcut olduğu için hata alırsınız
	// Keyleriniz her lisansa 1 adet bağlanır harici bağlantıda
	// kanallarınız açılmaz !!
	// Lütfen kodlarla oynamayınız giriş paketlerinizi bozabilirsiniz.
	unsigned long dwRunikUltimate = 1;
	for (size_t i = 0; i < strlen(pinfo->login); i++)
		dwRunikUltimate = (dwRunikUltimate * pinfo->login[i]) % 0xFFFFFFFF;
	
	unsigned long a = 928670843, b = 838670843;
	
	unsigned long ultimate_key = dwRunikUltimate ^ a;
	ultimate_key += b;
	ultimate_key >>= 3;
	ultimate_key |= 0xAA11;
	ultimate_key ^= b;
	
	if(ultimate_key != pinfo->ultimateDll)
	{
		sys_log(0, "Runik Anti Cheat Ultimate::Login : NOT_SECURITY_DLL");
