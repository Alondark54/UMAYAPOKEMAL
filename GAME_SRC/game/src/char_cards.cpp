...
void CHARACTER::Cards_open(DWORD safemode)
{
	...
	if (character_cards.cards_left <= 0) 
	{
		if (GetGold() < 30000)
		{
			ChatPacket(CHAT_TYPE_INFO, "Yeterli miktarda paran yok.");
			return;
		}
		if (CountSpecifyItem(79506) < 1)
		{
			ChatPacket(CHAT_TYPE_INFO, "Yeterli miktarda okey kart setin yok.");
			return;
		}
...
void CHARACTER::Cards_pullout()
{
	...
	if (empty_space == -1)
	{
		ChatPacket(CHAT_TYPE_INFO, "Bos yer yok.");
		return;
	}
	if (character_cards.cards_left < 1)
	{
		ChatPacket(CHAT_TYPE_INFO, "Kartin yok.");
		return;
	}
	if (GetAllCardsCount() >= 5)
	{
		ChatPacket(CHAT_TYPE_INFO, "Tabloda yer kalmadi.");
		return;
	}
	RandomizeCards();
	SendUpdatedInformations();
