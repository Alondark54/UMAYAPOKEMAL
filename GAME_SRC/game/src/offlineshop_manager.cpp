...
{
	if (!pkChr || !pkChrShopKeeper) return false;
	if (pkChr->GetOfflineShopOwner() == pkChrShopKeeper) return false;
	if (pkChrShopKeeper->IsPC()) return false;
	//sys_log(0, "OFFLINE_SHOP: START: %s", pkChr->GetName());
	return true;
}

LPOFFLINESHOP COfflineShopManager::CreateOfflineShop(LPCHARACTER npc, DWORD dwOwnerPID, TOfflineShopItemTable* pTable, short bItemCount, int32_t lMapIndex, int32_t iTime, const char* szSign, BYTE bBoardStyle)
{
	if (FindOfflineShop(npc->GetVID())) return nullptr;
	...
	m_map_pkOfflineShopByNPC.emplace(TShopMap::value_type(static_cast<DWORD>(npc->GetVID()), pkOfflineShop));
	m_Map_pkOfflineShopByNPC2.emplace(TOfflineShopMap::value_type(dwOwnerPID, static_cast<DWORD>(npc->GetVID())));
...
LPOFFLINESHOP COfflineShopManager::FindOfflineShop(DWORD dwVID)
{
	...
	if (it == m_map_pkOfflineShopByNPC.end()) return nullptr;
	return it->second;
}
...
void COfflineShopManager::DestroyOfflineShop(LPCHARACTER ch, DWORD dwVID, bool pcMode)
{
	if (pcMode) {
		...
		if (ch->GetMapIndex() >= 10000 || g_bChannel == 99) {
			ch->ChatPacket(CHAT_TYPE_INFO, "[LS:3585]");
			return;
		}
		*/
		...
		delete pkOfflineShop;
		if (ch && ch->GetDesc()) ch->ChatPacket(CHAT_TYPE_COMMAND, "BINARY_CloseOfflineShop");
	}
	else {
...
{
	if (!ch) return;
	if (thecore_pulse() - ch->GetMyOfflineShopTime() < PASSES_PER_SEC(2)) { ch->ChatPacket(CHAT_TYPE_INFO, "Lutfen 10 saniye bekle. Bu kadar hızlı olmak zararlı."); return; }
	//if (ch->GetMapIndex() >= 10000 || g_bChannel == 99) { ch->ChatPacket(CHAT_TYPE_INFO, "[LS:3585]"); return; }
	if (ch->m_pkTimedEvent) { ch->ChatPacket(CHAT_TYPE_INFO, "Lutfen 10 saniye bekle. Bu kadar hızlı olmak zararlı."); return; }
	ch->SetMyOfflineShopTime();
	quest::PC* pPC = quest::CQuestManager::Instance().GetPC(ch->GetPlayerID());
	if (pPC->IsRunning()) return;
	if (ch->HaveAnotherPagesOpen()) { ch->ChatPacket(CHAT_TYPE_INFO, "Acik olan pencereleri kapat ve tekrar dene."); return; }
	if (bDisplayPos >= OFFLINE_SHOP_HOST_ITEM_MAX_NUM) { sys_err("Overflow offline shop slot count [%s]", ch->GetName()); return; }

	LPITEM pkItem = ch->GetItem(bPos);
	if (!pkItem) return;
	const TItemTable* itemTable = pkItem->GetProto();
	if (IS_SET(itemTable->dwAntiFlags, ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_MYSHOP)) { ch->ChatPacket(CHAT_TYPE_INFO, "Giyili e?ya mı var? Yoksa bana bir ?eyler deniyorsun gibi mi geliyor."); return; }
	if (pkItem->isLocked()) { ch->ChatPacket(CHAT_TYPE_INFO, "[LS:3579]"); return; }
#ifdef ENABLE_SOULBIND_SYSTEM
	if (pkItem->IsSealed()) { ch->ChatPacket(CHAT_TYPE_INFO, "Ruha baglanmis esyayi pazara ekleyemezsin."); return; }
#endif
	if (pkItem->IsEquipped()) { ch->ChatPacket(CHAT_TYPE_INFO, "Kilitli bir esyayi pazara ekleyemezsin. Eklesen de pazari acamazsin zaten."); return; }
	{
		BYTE bChannel = ch->GetOfflineShopChannel();
		if (bChannel == 0) return;
...
void COfflineShopManager::RemoveItem(LPCHARACTER ch, BYTE bPos)
{
	...
	ch->SetMyOfflineShopTime();
	quest::PC* pPC = quest::CQuestManager::Instance().GetPC(ch->GetPlayerID());
	const int32_t iEmptyPos = ch->GetEmptyInventory(3);
	if (iEmptyPos < 0) { 
		ch->ChatPacket(CHAT_TYPE_INFO, "Once envanterinizde yer acin."); 
		return;
	}
	...
	if (pkOfflineShop->GetLeftItemCount(ch->GetPlayerID()) <= 0) npc->DestroyOfflineShop();
}
...
{
	if (!ch) return;
	auto pMsg(DBManager::instance().DirectQuery("SELECT shop_gold FROM player.player WHERE id = %u", ch->GetPlayerID()));
	TPacketGCShop p;
...
void COfflineShopManager::RefreshUnsoldItems(LPCHARACTER ch)
{
	...
	while (nullptr != (row = mysql_fetch_row(pMsg->Get()->pSQLResult))) {
		...
		for (uint8_t i = 0, n = 4; i < ITEM_SOCKET_MAX_NUM; ++i, n++) str_to_number(alSockets[i], row[n]);
		TPlayerItemAttribute aAttr[ITEM_ATTRIBUTE_MAX_NUM];
		for (uint8_t i = 0, iStartType = 4 + ITEM_SOCKET_MAX_NUM, iStartValue = 5 + ITEM_SOCKET_MAX_NUM; i < ITEM_ATTRIBUTE_MAX_NUM; ++i, iStartType += 2, iStartValue += 2) {
...
{
	if (!ch) return;
	if (bPos >= OFFLINE_SHOP_HOST_ITEM_MAX_NUM) return;
	if (ch->GetOfflineShop() || ch->GetOfflineShopOwner()) return;
	//if (ch->GetMapIndex() >= 10000 || g_bChannel == 99) { ch->ChatPacket(CHAT_TYPE_INFO, "[LS:3585]"); return; }
	...
	TPlayerItem item;
	uint8_t rows;
	if (!(rows = mysql_num_rows(pMsg->Get()->pSQLResult))) return;
...
DWORD COfflineShopManager::FindMyOfflineShop(DWORD dwPID)
{
	...
	if (m_Map_pkOfflineShopByNPC2.end() == it) return 0;
	return it->second;
}

void COfflineShopManager::ChangeOfflineShopTime(LPCHARACTER ch, int32_t bTime)
{
	if (!ch) return;
	DWORD dwOfflineShopVID = FindMyOfflineShop(ch->GetPlayerID());
	if (!dwOfflineShopVID) return;
	LPCHARACTER npc = CHARACTER_MANAGER::instance().Find(FindMyOfflineShop(ch->GetPlayerID()));
	if (npc) {
		int32_t iTime = bTime * 60;
		auto pMsg(DBManager::instance().DirectQuery("UPDATE %soffline_shop_npc SET time = time + %d WHERE owner_id = %u", get_table_postfix(), iTime, ch->GetPlayerID()));
		...
		// p.bTime = bTime;
		auto pMsg(DBManager::instance().DirectQuery("SELECT mapIndex FROM %soffline_shop_npc WHERE owner_id = %u", get_table_postfix(), ch->GetPlayerID()));
		if (pMsg->Get()->uiNumRows == 0) { ch->ChatPacket(CHAT_TYPE_INFO, "[LS:3590]"); return; }
...
void COfflineShopManager::StopShopping(LPCHARACTER ch)
{
	if (!ch || (ch && !ch->GetDesc())) return;
	LPOFFLINESHOP pkOfflineShop;
	if (!(pkOfflineShop = ch->GetOfflineShop())) return;
	...
	for (BYTE i = 0; i < OFFLINE_SHOP_HOST_ITEM_MAX_NUM; ++i) pack2.items[i].vnum = 0;
	pack.size = sizeof(pack) + sizeof(pack2);
	if (ch && ch->GetDesc()) {
		ch->GetDesc()->BufferedPacket(&pack, sizeof(TPacketGCShop));
		ch->GetDesc()->Packet(&pack2, sizeof(TPacketGCOfflineShopStart));
...
void COfflineShopManager::Buy(LPCHARACTER ch, BYTE pos)
{
	if (!ch || (ch && !ch->GetDesc())) return;
	if (!ch->GetOfflineShop() || !ch->GetOfflineShopOwner()) return;
	if (DISTANCE_APPROX(ch->GetX() - ch->GetOfflineShopOwner()->GetX(), ch->GetY() - ch->GetOfflineShopOwner()->GetY()) > 1500) { ch->ChatPacket(CHAT_TYPE_INFO, "[LS:3591]"); return; }
	LPOFFLINESHOP pkOfflineShop = ch->GetOfflineShop();
	if (!pkOfflineShop) return;
	int32_t ret = pkOfflineShop->Buy(ch, pos);
	if (SHOP_SUBHEADER_GC_OK != ret) {
		TPacketGCShop pack;
		pack.header = HEADER_GC_OFFLINE_SHOP;
		pack.subheader = ret;
		pack.size = sizeof(TPacketGCShop);
		if (ch && ch->GetDesc()) ch->GetDesc()->Packet(&pack, sizeof(pack));
	}
}
...
{
	if (!ch) return;
	long long llRequiredMoney = dwRequiredMoney;
	if (llRequiredMoney < 0) return;
	auto pMsg(DBManager::instance().DirectQuery("SELECT shop_gold FROM %splayer WHERE id = %u", get_table_postfix(), ch->GetPlayerID()));
	if (pMsg->Get()->uiNumRows == 0) return;
	long long llCurrentMoney = 0;
	MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
	str_to_number(llCurrentMoney, row[0]);
	if (llRequiredMoney > llCurrentMoney) llRequiredMoney = llCurrentMoney;
	bool isOverFlow = ch->GetGold() + llRequiredMoney >= ch->GetAllowedGold() ? true : false;
	if (isOverFlow) { ch->ChatPacket(CHAT_TYPE_INFO, "[LS:3592]"); return; }

	ch->PointChange(POINT_GOLD, llRequiredMoney, false);
	//ch->ChatPacket(CHAT_TYPE_INFO, "[LS:3593|%s]", pretty_number(llRequiredMoney).c_str());
...
BYTE COfflineShopManager::LeftItemCount(LPCHARACTER ch)
{
	...
	if (!npc) return -1;
	LPOFFLINESHOP pkOfflineShop = npc->GetOfflineShop();
	if (!pkOfflineShop) return -1;
	BYTE ret = pkOfflineShop->GetLeftItemCount(npc->GetOfflineShopRealOwner());
	return ret;
}
...
bool COfflineShopManager::HasOfflineShop(LPCHARACTER ch)
{
	...
	if (ch->GetOfflineShopVID()) {
		auto pMsg(DBManager::instance().DirectQuery("SELECT owner_id FROM %soffline_shop_npc WHERE owner_id = %u", get_table_postfix(), ch->GetPlayerID()));
		if (pMsg->Get()->uiNumRows == 0) {
			bHasOfflineShop = 0;
			if (ch && ch->GetDesc()) {
				ch->GetDesc()->BufferedPacket(&p, sizeof(TPacketGCShop));
...
{
	if (ch && (ch->HaveAnotherPagesOpen())) { ch->ChatPacket(CHAT_TYPE_INFO, "Acik olan pencereleri kapat ve tekrar dene."); return false; }
...
{
	LPOFFLINESHOP pOfflineShop = FindShop(dwVID);
	if (pOfflineShop) return pOfflineShop->IsOfflineShopNPC() ? pOfflineShop->GetOfflineShopNPC() : 0;
	else return 0;
}

int32_t COfflineShopManager::GetMapIndex(DWORD dwPID)
{
	LPCHARACTER npc = CHARACTER_MANAGER::instance().Find(FindMyOfflineShop(dwPID));
	if (!npc) return 0;
	LPOFFLINESHOP pkOfflineShop = npc->GetOfflineShop();
	if (!pkOfflineShop) return 0;
	return pkOfflineShop->GetOfflineShopMapIndex();
}

bool COfflineShopManager::GetOfflineShopCoords(DWORD dwPID, int32_t* map_index, int32_t* x, int32_t* y)
{
	LPCHARACTER npc = CHARACTER_MANAGER::instance().Find(FindMyOfflineShop(dwPID));
	if (!npc) return false;
	LPOFFLINESHOP pkOfflineShop = npc->GetOfflineShop();
	if (!pkOfflineShop) return false;
...
bool COfflineShopManager::GetOfflineShopCoordsP2P(LPCHARACTER ch, int32_t* map_index, int32_t* x, int32_t* y)
{
	if (!ch) return false;
	if (!ch->HasOfflineShop()) return false;
	DWORD player_id = ch->GetPlayerID();
	auto pMsg(DBManager::instance().DirectQuery("SELECT x,y,z,mapIndex FROM offline_shop_npc%s WHERE owner_id = %u", get_table_postfix(), player_id));
	if (!pMsg || (pMsg && pMsg->Get()->uiNumRows == 0)) return false;
	int32_t shop_x = -1, shop_y = -1, shop_z = -1, shop_MapIndex = -1;
	MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
...
int32_t COfflineShopManager::GetLeftTime(DWORD dwPID)
{
	LPCHARACTER npc = CHARACTER_MANAGER::instance().Find(FindMyOfflineShop(dwPID));
	if (!npc) return 0;
	LPOFFLINESHOP pkOfflineShop = npc->GetOfflineShop();
...
const char* COfflineShopManager::GetOfflineShopSign(DWORD dwPID)
{
	...
	LPCHARACTER npc = CHARACTER_MANAGER::instance().Find(FindMyOfflineShop(dwPID));
	if (!npc) return cleanSign;
	LPOFFLINESHOP pkOfflineShop = npc->GetOfflineShop();
	if (!pkOfflineShop) return cleanSign;
...
void COfflineShopManager::RemoveItemP2P(DWORD dwPID, BYTE bPos)
{
	...
	if (pkOfflineShop->GetLeftItemCount(dwPID) <= 0) npc->DestroyOfflineShop();
}

void COfflineShopManager::GiveItemP2P(DWORD dwPID, TPlayerItem item2)
{
	LPCHARACTER ch = CHARACTER_MANAGER::instance().FindByPID(dwPID);
	if (!ch) { sys_err("CRITICAL ERROR: GiveItemP2P PID: %d", dwPID); return; }

	LPITEM pItem = ITEM_MANAGER::instance().CreateItem(item2.vnum, item2.count, item2.id);
	if (!pItem) { ch->ChatPacket(CHAT_TYPE_INFO, "[LS:3589]"); return; }
	...
	if (!ch) {
		...
		if (pcci && pcci->pkDesc) {
			if (pcci->lMapIndex >= 10000 || pcci->bChannel == 99) {
				pcci->pkDesc->SetRelay(pcci->szName);
				pcci->pkDesc->ChatPacket(CHAT_TYPE_INFO, "[LS:3585]");
				pcci->pkDesc->SetRelay("");
			}
		}
		else
...
void COfflineShopManager::ChangeOfflineShopName(DWORD dwPID, const char* shopName)
{
	...
	const uint8_t borderStyle = pkOfflineShop->GetBorderStyle();
	npc->ChangeShopSign(npc->GetVID(), shopName, borderStyle);
...
void COfflineShopManager::ChangeOfflineShopItemPrice(LPCHARACTER ch, BYTE bPos, long long llPrice)
{
	...
	LPOFFLINESHOP pkOfflineShop = npc->GetOfflineShop();
	if (!pkOfflineShop) return;
	if (pkOfflineShop->ChangeItemPrice(ch->GetPlayerID(), bPos, llPrice)) ch->ChatPacket(CHAT_TYPE_INFO, "[LS:3595]");
}
...
void COfflineShopManager::ChangeOfflineShopItemPriceP2P(DWORD dwPID, BYTE bPos, long long llPrice)
{
	...
	if (pkOfflineShop->ChangeItemPrice(dwPID, bPos, llPrice)) {
		...
		if (!ch) {
			...
			if (pcci && pcci->pkDesc) {
				pcci->pkDesc->SetRelay(pcci->szName);
				pcci->pkDesc->ChatPacket(CHAT_TYPE_INFO, "[LS:3595]");
				pcci->pkDesc->SetRelay("");
			}
		}
		else {
			if (ch->GetDesc()) ch->ChatPacket(CHAT_TYPE_INFO, "[LS:3595]");
		}
	}
}
