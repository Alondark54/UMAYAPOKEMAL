...
class ItemToolTip(ToolTip):
	...
	def __init__(self, *args, **kwargs):
		...
		if app.ENABLE_OFFLINE_SHOP_SYSTEM: self.isOfflineShopItem = (False, -1)
	...
	if app.ENABLE_AUTOMATIC_ITEM_PROCESS:
		def SetDeleteItemEx(self, itemVnum):
			item.SelectItem(itemVnum)
			self.ClearToolTip()
	...
	if app.ENABLE_OFFLINE_SHOP_SYSTEM:
		...
		def SetOfflineShopItem(self, slotIndex):
			...
			self.AddItemData(itemVnum, metinSlot, attrSlot, 0, 0, player.ON_TOP_WND_OFFLINE_SHOP, slotIndex)
			self.AppendPrice(price)
	...
	if app.ITEM_SHOP:
		def SetItemTooltipIS(self, itemVnum, metinSlot, attrSlot):
			self.ClearToolTip()
			self.AddItemData(itemVnum, metinSlot, attrSlot)
			if app.__BL_CHEST_DROP_INFO__:
				self.AppendChestDropInfo(itemVnum)
	...
	def AddItemData_Offline(self, itemVnum, itemDesc, itemSummary, metinSlot, attrSlot):
		...
		if item.ITEM_TYPE_NEW_BLEND == itemType and app.__NEW_POTION__:
			if itemSubType == item.BLEND_TIME:
				self.AppendSpace(5)
				self.AppendTextLine("Time" + " : " + localeInfo.FormatTime(item.GetValue(0) * 60), self.NORMAL_COLOR)
			elif itemSubType == item.BLEND_POTION:
				self.__AppendAffectInformation()

				endTime = metinSlot[1] if metinSlot[0] else metinSlot[1] + app.GetGlobalTimeStamp()
				leftSec = max(0, endTime - app.GetGlobalTimeStamp())
				self.AppendSpace(5)
				self.AppendTextLine(localeInfo.LEFT_TIME + " : |cFFFFFF00" + localeInfo.FormatTime(leftSec), self.NORMAL_COLOR)
...
#Add
	...
	def AdditionalTips(self, window_type, itemVnum, metinSlot, slotIndex):
		...
		if app.ENABLE_EMOJI_SYSTEM and item.IsFlag(item.ITEM_FLAG_STACKABLE) and window_type in (player.INVENTORY,):
			if player.GetItemCount(window_type, slotIndex) > 1:
				self.AppendTextLine("{} + {} {}".format(Emoji("icon\emoji\key_shift.png"),  Emoji("icon\emoji\key_lclick.png"), "Bolunebilir Item"))
	...
	if app.__BL_CHEST_DROP_INFO__:
		def AppendChestDropInfo(self, itemVnum):
			...
			if hasinfo:
				self.AppendSpace(5)
				self.AppendTextLine(localeInfo.HIZLIAC, self.NORMAL_COLOR)
				self.AppendSpace(5)
