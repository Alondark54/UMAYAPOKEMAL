...
class EigenartigSwitchBotDialog(ui.ScriptWindow):
	...
	def __del__(self):
		...
		if constInfo.yesilefsunbotu:
			constInfo.yesilefsunbotu = 0
		
	def AddGui(self):
		Gui = [
		[[ui.BoardWithTitleBar, ""], [495-58, 285], [0,0], [["SetCenterPosition", [""]], ["SetCloseEvent", [self.__del__]], ["SetTitleName", ["Yeþil Efsun Botu "]]], ["float","movable"]],			
		[[ui.ImageBox, 0], [0, 0], [10, 35], [['LoadImage', ["system/efsunbot/page1.dds"]]], []],
		]
		
		for i in xrange(5):
			x =82*i
			button = [[ui.Button, 0], [80, 27], [x, 35], [['SetEvent', [lambda Index = i: self.SelectTab(Index)]]], []]
			Gui.append(button)		
		
		self.lenGui = len(Gui)
		
		for i in xrange(5):
			tab =  [[TabGui, 0], [470, 320], [30, 90], [["SetBtnEvents", [self.StartBOT]]], []]
			Gui.append(tab)
	...
	def SelectTab(self, Index):
		...
		self.Gui[1].LoadImage("system/efsunbot/page%s.dds" % (Index+1))
	...
	def StartBOT(self, type):
		...
		if not type:	
			if not self.Cfg[Tab][0]:
				...
				for Slot in xrange(player.INVENTORY_PAGE_SIZE*200):
				...
				self.DefineBoni()
				chat.AppendChat(1, "Efsun botu baþladý.")
				self.Cfg[Tab][0] = 1			
				self.Gui[self.lenGui+Tab].SetbtnText(self.Cfg[Tab][0])
			else:
				self.Cfg[Tab][0] = 0
				chat.AppendChat(1, "Efsun botu durdu.")
				self.Gui[self.lenGui+Tab].SetbtnText(self.Cfg[Tab][0])
		else:
	...
	def ControllBoni(self, Boni, Values, tabNumber):
		try:
			...
			chat.AppendChat(1, "Efsunlama iþlemi baþarýlý.")
		except:
			if player.GetItemCountByVnum(71151) <= 0:
				if shop.IsOpen():
					for EachShopSlot in xrange(shop.SHOP_SLOT_COUNT):
						ShopItemValue = shop.GetItemID(EachShopSlot)
						if ShopItemValue == 71151:
	...
	def UpdateBoni(self, Index):
		...
		if (player.GetItemCount(self.Cfg[Index][1][0]) <= 0 and (self.Cfg[Index][3] != Boni or self.Cfg[Index][4] != Values)) or (self.Cfg[Index][3] != Boni or self.Cfg[Index][4] != Values) or app.GetTime() >= self.Cfg[Index][2] + 1.2:
	...
	def UpdateBoni67(self, Index):
		...
		if (player.GetItemCount(self.Cfg[Index][8][0]) <= 0 and (self.Cfg[Index][10] != Boni or self.Cfg[Index][11] != Values)) or (self.Cfg[Index][10] != Boni or self.Cfg[Index][11] != Values) or app.GetTime() >= self.Cfg[Index][9] + 1.2:
...
##		
	...
	def __init__(self):
		...
		self.BonusIDListe = [["Seçilmedi", 0], ["Max HP.", 1], ["Max SP.", 2], ["Canlýlýk", 3], ["Zeka", 4], ["Güç", 5], ["Çeviklik", 6], ["Saldýrý Hýzý", 7], ["Hareket Hýzý", 8], ["Büyü Hýzý", 9], ["Hp Üretimi", 10], ["SP Üretimi", 11], ["Zehirleme Deðiþimi", 12], ["Sersemletme Þansý", 13], ["Yavaþlatma Þansý", 14], ["Kritik Vuruþ", 15], ["Delici Vuruþ", 16], ["Yarý Ýnsanlara Karþý Güçlü", 17], ["Hayvanlara Karþý Güçlü", 18], ["Orklara Karþý Güçlü", 19], ["Mistiklere Karþý Güçlü", 20], ["Ölümsüzlere Karþý Güçlü", 21], ["Þeytanlara Karþý Güçlü", 22], ["HP Absorbe", 23], ["SP Absorbe", 24], ["Beden K. Ataklarýn Blok.", 27], ["Oklardan Korunma Þansý", 28], ["Kýlýç Savunmasý", 29], ["Çift El Savunmasý", 30], ["Býçak Savunmasý", 31], ["Çan Savunmasý", 32], ["Yelpaze Savunmasý", 33], ["Oka Karþý Dayanýklýlýk", 34], ["Ateþe Karþý Dayanýklýlýk", 35], ["Þimþeðe Karþý Dayanýklýlýk", 36], ["Büyüye K. Dayanýklýlýk", 37], ["Rüzgara K. Dayanýklýlýk", 38], ["Vücut D. Yansýtmasý", 39], ["Zehre Karþý Koyma", 41], ["Exp-Bonus", 43], ["Ýki Kat Yang", 44], ["Ýki Kat Ýtem", 45], ["Sersemlik K. Baðýþýklýlýk", 48], ["Yavaþlama K. Baðýþýklýlýk", 49], ["Saldýrý Deðeri", 53], ["Beceri Hasarý", 71], ["Ortalama Hasar", 72]]
		self.BonusIDListe2 = [["Seçilmedi", 0], ["Max HP.", 1], ["Max SP.", 2], ["Canlýlýk", 3], ["Zeka", 4], ["Güç", 5], ["Çeviklik", 6], ["Kritik Vuruþ", 15], ["Delici Vuruþ", 16], ["Saldýrý Deðeri", 53], ["Savaþçýya K. Güçlü", 59], ["Ninjaya K. Güçlü", 60], ["Suraya K. Güçlü", 61], ["Þamana K. Güçlü", 62], ["Beceri Hasarý", 71], ["Ortalama Hasar", 72]]
		...
		self.gridSlowWind.SetPosition(18,5)
		...
		self.delete_bonus.SetUpVisual("system/efsunbot/efsun_bot_button_1.tga")
		self.delete_bonus.SetOverVisual("system/efsunbot/efsun_bot_button_2.tga")
		self.delete_bonus.SetDownVisual("system/efsunbot/efsun_bot_button_3.tga")
		self.delete_bonus.SetText("Temizle")
		self.delete_bonus.SetPosition(285-144-123, 250+49-161)
		...
		self.save_bonus.SetUpVisual("system/efsunbot/efsun_bot_button_1.tga")
		self.save_bonus.SetOverVisual("system/efsunbot/efsun_bot_button_2.tga")
		self.save_bonus.SetDownVisual("system/efsunbot/efsun_bot_button_3.tga")
		self.save_bonus.SetText("Kaydet")
		self.save_bonus.SetPosition(285-144+123, 250+49-161)
		...
		self.six_seven_bonus.SetUpVisual("d:/ymir work/ui/public/Middle_thin_button_01.sub")
		self.six_seven_bonus.SetOverVisual("d:/ymir work/ui/public/Middle_thin_button_01.sub")
		self.six_seven_bonus.SetDownVisual("d:/ymir work/ui/public/Middle_thin_button_01.sub")
		self.six_seven_bonus.SetText("s")
		self.six_seven_bonus.SetPosition(285-144, 250+49-161)
		...
		self.norm_bonus.SetUpVisual("system/efsunbot/efsun_bot_button_1.tga")
		self.norm_bonus.SetOverVisual("system/efsunbot/efsun_bot_button_2.tga")
		self.norm_bonus.SetDownVisual("system/efsunbot/efsun_bot_button_3.tga")
		self.norm_bonus.SetText("Baþlat")
		self.norm_bonus.SetPosition(285-144, 250+49-161)
		...
		self.load_bonus.SetPosition(3, 201-98)
		...
		for i in xrange(2):
			...
			selectBonus.SetCurrentItem("s")
			selectBonus.InsertList(self.BonusIDListe2)
			selectBonus.SetSize(220, 18)
			selectBonus.SetPosition(90, yPos)
			...
			slotBar.SetSize(41, 18)
			slotBar.SetPosition(410+38-127, y_Pos)
			slotBar.Show()
		
			editLine = ui.EditLine()
			editLine.SetParent(slotBar)
			editLine.SetSize(41, 17)
	...
	def LoadBnsCfgFile(self):
		self.SavedBonus = []
		try:
			for information in open("efsunbotyesil.eig", "r").readlines():
	...
	def LoadBonusSets(self, index):
		if index ==31:
			try: os.remove("efsunbotyesil.eig")
			except: pass
			chat.AppendChat(1,"Tüm kayýtlý ayarlar silindi.")
			self.LoadBnsCfgFile()
	...
	def SaveBonusSet(self):
		...
		dosya = open("efsunbotyesil.eig", "a+")
