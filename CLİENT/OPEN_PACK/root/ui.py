...
class InvisibleBoard(Window):
	...
	def SetSize(self, width, height):
		if not self.skipMaxCheck:
			width = max(self.CORNER_WIDTH, width)
			height = max(self.CORNER_HEIGHT, height)
...
class CheckBoxIbo(Window):
	...
	def CreateElements(self):
		...
		self.backgroundImage.SetParent(self)
		self.backgroundImage.AddFlag("not_pick")
		self.backgroundImage.LoadImage("ibowork/check/quest_checkbox.tga")
		self.backgroundImage.Show()
		...
		self.checkImage.LoadImage("ibowork/check/quest_checked2.tga")
		self.checkImage.Hide()
