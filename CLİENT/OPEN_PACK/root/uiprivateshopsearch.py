...
def DeletePrivateShopEffect():
	...
	constInfo.CREATE_PRIVATE_SHOP_EFFECT = (0, False, app.GetTime())

def CreatePrivateShopEffect(chrVid):
	background.CreatePrivateShopPos(chrVid)
	constInfo.CREATE_PRIVATE_SHOP_EFFECT = (chrVid, True, app.GetTime() + 180)
...
class PrivateShopSeachWindow(ui.ScriptWindow):
	...
	if app.ENABLE_WOLFMAN_CHARACTER:
		...
		JOB_NAME_DIC = {	0: localeInfo.JOB_WARRIOR,
						 ...
						 99: localeInfo.PRIVATESHOPSEARCH_ALL,
						 }
	else:
		JOB_MAX_COUNT = 4
		JOB_NAME_DIC = {	0: localeInfo.JOB_WARRIOR,
	...
	def __init__(self):
		...
		self.saveitemMasktypeselectnumber = -1

		self.savejobselectnumber = 99
		self.saveitemMasksubtypeselectnumber = -1
	...
	def __MakeResultSlot(self):
		...
		# self.itemMasktypeselectslot.SetCurrentItem(
			# self.ITEM_MASK_TYPE_DIC[item.ITEM_TYPE_NONE])
		...
		self.jobselectslot.InsertItem(99, self.JOB_NAME_DIC[99])
		self.jobselectslot.SetCurrentItem(self.JOB_NAME_DIC[99])
	...
	def RefreshList(self):
		...
		for line, nameSlotList in self.SerchItemSlotList.items():
			if app.ENABLE_CHEQUE_SYSTEM:
				itemname, sellername, count, price, cheque, shopID = shop.GetPrivateShopSearchResult(
					line)
				if "" == itemname:
					...
					nameSlotList[3].SetText(str(cheque))
					self.ItemSlotButtonList[line][0].Show()
			else:
				itemname, sellername, count, price, shopID = shop.GetPrivateShopSearchResult(
					line)
				if "" == itemname:
					nameSlotList[0].SetText("")
					nameSlotList[1].SetText("")
	...
	def Search(self):
		...
		if len(self.maxgoldvalue.GetText()) <= 0:
			self.maxgoldvalue.SetText("4000000000")
		...
		if app.ENABLE_CHEQUE_SYSTEM:
			...
			net.SendPrivateShopSearchInfo(job, Masktype, Masksub, 0, 0,
										  0, 0, mingold, maxgold, itemname, mincheque, maxcheque)
		else:
			net.SendPrivateShopSearchInfo(
				job, Masktype, Masksub, 0, 0, 0, 0, mingold, maxgold, itemname)
	...
	def __SelectItemSlotButtonList(self, index):
		for line, ItemSlotButtonList in self.ItemSlotButtonList.items():
			if index != line:
				...
				itemname, sellername, count, price, shopID = shop.GetPrivateShopSearchResult(line)
				CreatePrivateShopEffect(shopID)
