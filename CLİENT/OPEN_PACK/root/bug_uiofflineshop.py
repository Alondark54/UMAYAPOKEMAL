...
#				new_index += 1
	...
	def SelectEmptySlot(self, selectedSlotPos):
		...
		if isAttached:
			...
			# if app.ENABLE_SOUL_BIND_SYSTEM:
				# if player.GetItemSealDate(attachedInvenType, attachedSlotPos) == -1 or player.GetItemSealDate(attachedInvenType, attachedSlotPos) > 0: #∫¿¿Œæ∆¿Ã≈€ ∞…∑Ø≥ø.
					# return True
	...
	def UnselectItemSlot(self, selectedSlotPos):
		...
		if (app.IsPressed(app.DIK_LCONTROL) or app.IsPressed(app.DIK_RCONTROL)) and\
			...
			if not self.isOpen: self.AskRemoveItem(selectedSlotPos)
	...
	def OverInItem(self, slotIndex):
		...
		if None != selfs.tooltipItem:
			...
			if shop.GetOfflineShopItemStatus(slotIndex) < 2 and not g_isEditingOfflineShop: 
