...
class BattlePassWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.GetChild("complate_button_ep").SetEvent(ui.__mem_func__(self.__OnClickComplateButton), 0)
			self.GetChild("complate_button_item").SetEvent(ui.__mem_func__(self.__OnClickComplateButton), 1)
	...
	def __OnClickComplateButton(self, type):
		battlePassType = 1
		if self.page == "PREMIUM": battlePassType = 2
		if self.page == "EVENT": battlePassType = 3
		net.SendExtBattlePassComplateMission(type, battlePassType, self.mission_index)
	...
	def SetPremiumPage(self):
		...
		if self.battlePassGeneralInfo[2].has_key(self.BattlePassIDPremium) and self.battlePassGeneralInfo[2][self.BattlePassIDPremium]["start_time"] < app.GetGlobalTimeStamp() and self.battlePassGeneralInfo[2][self.BattlePassIDPremium]["end_time"] > app.GetGlobalTimeStamp():
			if player.GetPremiumBattlePassID() != self.BattlePassIDPremium:
				...
				self.GetChild("InputTicketInfoTimeText2").SetText(datetime.fromtimestamp(self.battlePassGeneralInfo[2][self.GetActualBattlePassIDByType()]["start_time"]).strftime('%d.%m.%Y %H:%M:%S') + " - " + datetime.fromtimestamp(self.battlePassGeneralInfo[2][self.GetActualBattlePassIDByType()]["end_time"]).strftime('%d.%m.%Y %H:%M:%S'))
	...
	def SetMissionInfo(self, mission_index, mission_type):
		self.mission_index = int(mission_index)
