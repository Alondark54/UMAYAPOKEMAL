...
class CostumeWindow(ui.ScriptWindow):
	...
	if app.ENABLE_HIDE_COSTUME_SYSTEM:
		def RefreshVisibleCostume(self):
			...
			if body == 1:
				self.visibleButtonList[0].SetToolTipText("GOSTER")
				self.visibleButtonList[0].Down()
			else:
				self.visibleButtonList[0].SetToolTipText("GIZLE")
				self.visibleButtonList[0].SetUp()

			if hair == 1:
				self.visibleButtonList[1].SetToolTipText("GOSTER")
				self.visibleButtonList[1].Down()
			else:
				self.visibleButtonList[1].SetToolTipText("GIZLE")
		...
		def VisibleCostume(self, part, hidden):
			...
			if currentTime - self.lastButtonPressTime >= 30:
				self.lastButtonPressTime = currentTime
				self.blockButtonPressUntil = currentTime + 30
				net.SendChatPacket("/hide_costume %d %d" % (part, hidden))
			else:
				remainingTime = self.blockButtonPressUntil - currentTime
				chat.AppendChat(chat.CHAT_TYPE_INFO, "Kostumun yeniden gorunur hale gelmesi icin %d saniye bekleyin." % remainingTime)
	...
	class FastInventoryWindow(ui.ScriptWindow):
		...
		def ClickMenu3(self):
			if not constInfo.yesilefsunbotu:
				...
				chat.AppendChat(1, "Yesil Efsun botu zaten aktif.")
		def ClickMenu4(self):
			if self.wndInventory.interface:
