...
#				return
	...
	def __SendShoutChatPacket(self, text):
		...
		if self.lastShoutTime and app.GetTime() < self.lastShoutTime + 7: #@fixme013
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.CHAT_SHOUT_LIMIT)
			self.__ResetChat()
			return

		#self.__SendChatPacket(text[1:], chat.CHAT_TYPE_SHOUT)
		self.__SendChatPacket(constInfo.Vectors+'|cFFFF0000|HVectors:'+str(player.GetName())+'|h[PM]|h|r'+" : "+text[1:], chat.CHAT_TYPE_SHOUT)
		self.__ResetChat()
...
class ChatWindow(ui.Window):
	...
	def __init__(self):
		...
		checkBox.SetEvent(ui.__mem_func__(self.StopChat), "ON_CHECK", True)
		checkBox.SetEvent(ui.__mem_func__(self.StopChat), "ON_UNCKECK", False)
		checkBox.Hide()
		self.checkBox = checkBox
		
		if app.AUTO_SHOUT:
			self.interface = None
			self.btnShout = ui.Button()
			self.btnShout.SetParent(self)
			self.btnShout.SetUpVisual("d:/ymir work/ui/game/taskbar/Open_Chat_Log_Button_01.sub")
			self.btnShout.SetOverVisual("d:/ymir work/ui/game/taskbar/Open_Chat_Log_Button_02.sub")
			self.btnShout.SetDownVisual("d:/ymir work/ui/game/taskbar/Open_Chat_Log_Button_03.sub")
			self.btnShout.SetToolTipText("Oto Baðýrma")
			self.btnShout.SetEvent(lambda : self.__AutoShout())
			self.btnShout.Hide()
...
# CHAT UYARI
		self.log_Board = ui.ThinBoard()
		self.log_Board.SetParent(self.btnChatSizing)
		self.log_Board.SetSize(230, 40) # Panel Boyutu
		self.log_Board.SetWindowHorizontalAlignCenter()
		self.log_Board.SetPosition(0, -35) # -35 Deðeri Yukarý Aþaðý
		self.log_Board.Show()
		self.comp = Component()

		duyuru = ("Ailevi küfür süresiz ban sebebidir.")	# Üstteki yazý
		self.text = self.comp.TextLine(self.log_Board, duyuru, 115, 8, self.comp.RGB(255, 150, 0))
		self.text.SetHorizontalAlignCenter() 
		duyuru2 = ("Bildirimlerde video kaydý zorunludur!")		# Alttaki Yazý
		self.text2 = self.comp.TextLine(self.log_Board, duyuru2, 115, 22, self.comp.RGB(153, 69, 237))
	...
	def StopChat(self, checkType, autoFlag):
		if autoFlag == 1:
			net.SendChatPacket("/chat_durdur 1")
		else:
			net.SendChatPacket("/chat_durdur 0")
	...
	def OpenChat(self):
		...
		self.checkBox.SetPosition(self.GetWidth() -25, 4)
		self.checkBox.Show()
		
		if app.AUTO_SHOUT:
			self.btnShout.SetPosition(self.GetWidth()-100, 2)
			self.btnShout.Show() #Show
